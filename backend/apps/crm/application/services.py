...
class ClientService(BaseService):
    ...
    def delete_client(cls, request, client: Client):
        cls.validate_ownership(client, request)
        client.delete()  # Soft delete via BaseModel
        AuditService.log_action(
            request,
...
class LeadService(BaseService):
    ...
    @classmethod
    @transaction.atomic
    def convert_lead(cls, request, lead: Lead) -> Deal:
        ...
        client = Client.objects.filter(tenant=tenant, email=lead.email).first() if lead.email else None
        if not client:
            client_name = lead.company if lead.company else f"{lead.first_name} {lead.last_name}"
        ...
        deal = Deal.objects.create(
            tenant=tenant,
            title=f"Opportunity - {client.name}",
            client=client,
            amount=lead.value,
            stage='prospecting',
        ...
        AuditService.log_action(
            ...
            resource=f"crm.Lead:{lead.pk}",
            payload={"new_deal": str(deal.pk), "client": str(client.pk)},
        )
...
class LeadScoringService(BaseService):
    ...
    - Call: 20 pts
    - Email: 5 pts
    - High Value (>10k): 100 pts
    """
    @classmethod
    def calculate_score(cls, lead: Lead) -> int:
        score = 0
        
        # 1. Activity Based Scoring
        activities = Activity.objects.filter(lead=lead)
        for act in activities:
            if act.activity_type == 'meeting': score += 50
            elif act.activity_type == 'call': score += 20
            elif act.activity_type == 'email': score += 5
            elif act.activity_type == 'note': score += 1

        # 2. Value Based Scoring
        if lead.value >= 10000:
            score += 100
        elif lead.value >= 1000:
            score += 25
            
        # 3. Status Based Penalties/Bonuses
    ...
    @classmethod
    def update_lead_score(cls, lead_id: str):
        try:
            lead = Lead.objects.get(id=lead_id)
            new_score = cls.calculate_score(lead)
