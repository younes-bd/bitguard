...
class PermissionTests(TestCase):
    def setUp(self):
        ...
        self.tenant = Tenant.objects.create(name="Test Tenant", domain="test_tenant.com")
        ...
        # Create users
        self.superuser = User.objects.create_superuser('super@test.com', 'super@test.com', 'pass')
        self.superuser.roles.add(self.super_admin_role)
        
        self.tenant_admin = User.objects.create_user('admin@test.com', 'admin@test.com', 'pass')
        self.tenant_admin.roles.add(self.tenant_admin_role)
        
        self.employee = User.objects.create_user('emp@test.com', 'emp@test.com', 'pass')
        self.employee.roles.add(self.employee_role)
        
        from apps.users.domain.models import TenantMembership
        TenantMembership.objects.create(user=self.superuser, tenant=self.tenant)
        TenantMembership.objects.create(user=self.tenant_admin, tenant=self.tenant)
        TenantMembership.objects.create(user=self.employee, tenant=self.tenant)
