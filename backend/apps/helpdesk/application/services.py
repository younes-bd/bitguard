...
class TicketService(BaseService):
    ...
    def create_ticket(cls, request, data: dict) -> Ticket:
        ...
        AuditService.log_action(
            ...
            resource=f"helpdesk.Ticket:{ticket.pk}",
    ...
    @classmethod
    @transaction.atomic
    def escalate_to_problem(cls, request, ticket: Ticket) -> Ticket:
        """
        Charter §25: Critical support incidents can be escalated to ITSM Problems.
        """
        cls.validate_ownership(ticket, request)
        tenant = cls.get_tenant_context(request)
        
        problem = Ticket.objects.create(
            tenant=tenant,
            title=f"Root Cause Analysis: {ticket.title}",
        ...
        ticket.parent_ticket = problem
        ticket.save(update_fields=['parent_ticket'])
