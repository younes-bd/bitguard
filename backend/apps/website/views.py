...
from .serializers import AnnouncementSerializer, SignupSerializer, WebsiteInquirySerializer, ServicePageSerializer, PageSerializer
...
class PageViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [permissions.AllowAny] # Service handles logic
    lookup_field = 'slug'

    def get_queryset(self):
        from .services import PageService
        return PageService().get_queryset(self.request)

    def create(self, request, *args, **kwargs):
        from .services import PageService
        try:
            page = PageService().create(request, request.data)
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            page = PageService().update(request, page_obj.id, request.data)
            serializer = self.get_serializer(page)
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        page_obj = self.get_object()
        try:
            PageService().delete(request, page_obj.id)
