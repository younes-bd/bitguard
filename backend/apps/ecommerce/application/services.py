...
class CommerceService(BaseService):
    ...
    def update_customization(cls, data, request):
        ...
        cust.save()
        AuditService.log_action(request, action="STORE_CUSTOMIZATION_UPDATED", resource=f"ecommerce.StoreCustomization:{cust.pk}", payload=data)
        return cust
    ...
    def create_category(cls, data, request):
        ...
        AuditService.log_action(request, action="STORE_CATEGORY_CREATED", resource=f"ecommerce.Category:{category.pk}", payload=data)
        return category
    ...
    def create_product(cls, data, request):
        ...
        AuditService.log_action(request, action="STORE_PRODUCT_CREATED", resource=f"ecommerce.Product:{product.pk}", payload=data)
        return product
    ...
    def create_order(cls, data, request):
        ...
        # Log Audit
        AuditService.log_action(request, action="STORE_ORDER_CREATED", resource=f"ecommerce.Order:{order.pk}", payload={"total_amount": float(order.total_amount)})
    ...
    def update_order_status(cls, order, new_status, request):
        ...
        if new_status == 'paid':
            ...
            try:
                ...
                print(f"ERP Integration Error: {e}")
                pass # Soft failure for workflow if ERP invoice fails

            # Enterprise Integration: Autonomous Stock Deduction
            for item in order.items.all():
                if item.product and item.product.track_stock:
                    ...
                    # 2. Decrement SCM Warehouse stock if linked
                    try:
                        ...
                        pass # SCM module might not be active

        AuditService.log_action(request, action="STORE_ORDER_STATUS_UPDATED", resource=f"ecommerce.Order:{order.pk}", payload={"status": new_status})
        return order
    ...
    def create_subscription(cls, data, request):
        ...
        AuditService.log_action(request, action="STORE_SUBSCRIPTION_CREATED", resource=f"ecommerce.Subscription:{sub.pk}", payload={"plan": sub.plan.name})
        return sub
