...
class Project(TenantAwareModel):
    """
    Dedicated Project Management model â€” separate from ERP InternalProject.
    ...
    progress_override = models.IntegerField(null=True, blank=True, help_text='0â€“100. Auto-computed from tasks if null.')
...
class Task(TenantAwareModel):
    """
    Kanban card â€” belongs to a Project and sits in a stage column.
    """
    PRIORITY_CHOICES = [
        ('low', 'Low'),
        ('medium', 'Medium'),
...
class TimeLog(TenantAwareModel):
    """
    Time tracking entry â€” logged against a task.
    ...
    def __str__(self):
        return f'{self.user} â€” {self.hours}h on {self.task.title}'
