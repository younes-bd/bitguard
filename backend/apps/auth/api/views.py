...
class VerifyOTPView(APIView):
    ...
    def post(self, request):
        ...
        AuditService.log_action(
            ...
            resource=f"users.User:{serializer.validated_data.get('user_id')}",
            payload={"method": "OTP"},
        )
        return standard_response(True, "MFA verification successful", serializer.validated_data)
...
class CustomTokenObtainPairView(TokenObtainPairView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            serializer.is_valid(raise_exception=True)
            user_id = serializer.validated_data.get('user', {}).get('id', 'unknown')
        except Exception as e:
            logger.error(f"Login validation failed: {str(e)} | Data: {request.data}")
            return standard_response(False, f"Login failed: {str(e)}", errors={"error": str(e)}, status=400)
...
class CustomTokenRefreshView(TokenRefreshView):
    def post(self, request, *args, **kwargs):
        ...
        return standard_response(True, "Token refreshed", {"access_token": serializer.validated_data.get('access')})
    ...
    def post(self, request):
        try:
            ...
            if refresh_token:
                ...
                token.blacklist()
            AuditService.log_action(request, action="USER_LOGOUT", resource_type="users.User", resource_id=request.user.pk)
