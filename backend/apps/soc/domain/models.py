...
class Incident(TenantAwareModel):
    ...
    external_event_id = models.CharField(max_length=255, blank=True, null=True, unique=True, help_text="ID from external SIEM/System")
...
class LogAnalysis(TenantAwareModel):
...
class ManagedEndpoint(TenantAwareModel):
    """
...
class CloudApp(TenantAwareModel):
    """
...
class SystemMonitor(TenantAwareModel):
    """
...
class NetworkEvent(TenantAwareModel):
    """
...
class CloudIntegration(TenantAwareModel):
    """
...
class RemoteSession(TenantAwareModel):
    """
...
class ComplianceFramework(TenantAwareModel):
    FRAMEWORK_CHOICES = [
        ('soc2', 'SOC 2'),
        ('iso27001', 'ISO 27001'),
        ('gdpr', 'GDPR'),
...
class ComplianceControl(TenantAwareModel):
    STATUS_CHOICES = [
        ('compliant', 'Compliant'),
        ('partial', 'Partial'),
    ...
    control_id = models.CharField(max_length=50, help_text="e.g. CC6.1, A.9.1")
    title = models.CharField(max_length=255)
    description = models.TextField(blank=True)
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='non_compliant')
    evidence = models.TextField(blank=True, help_text="Evidence notes or document reference")
    owner = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL,
        null=True, blank=True, related_name='compliance_controls'
    )
