...
import {
    LayoutDashboard, Users, Ticket, FileText,
    MessageSquare, Briefcase, ShoppingCart,
...
const CrmLayout = () => {
    ...
    const [user, setUser] = useState({ first_name: 'Agent', last_name: 'User', email: 'agent@bitguard.com' });
    ...
    const crmItems = [
        { icon: LayoutDashboard, label: 'Dashboard', path: '/admin/crm' },
        { icon: Users, label: 'Clients', path: '/admin/crm/clients' },
        { icon: Briefcase, label: 'Deals', path: '/admin/crm/deals' },
        { icon: ShoppingCart, label: 'Sales Orders', path: '/admin/crm/orders' },
        { icon: Ticket, label: 'Support Tickets', path: '/admin/crm/tickets' },
        { icon: FileText, label: 'Contracts', path: '/admin/crm/contracts' },
        { icon: MessageSquare, label: 'Interactions', path: '/admin/crm/interactions' },
    ];

    return (
        <div className="min-h-screen bg-slate-900 flex text-slate-100 font-sans">
            <Sidebar title="CRM Suite" items={crmItems} backLink="/admin" collapsed={sidebarCollapsed} setCollapsed={setSidebarCollapsed} />
            ...
            <main className={`flex-1 relative transition-all duration-300 ease-in-out ${sidebarCollapsed ? 'md:ml-20' : 'md:ml-64'}`}>
                ...
                <header className="h-16 border-b border-slate-800 bg-slate-900/80 backdrop-blur-md sticky top-0 z-30 flex items-center justify-between px-6">
                    ...
                    <div className="hidden md:flex items-center text-sm font-medium text-slate-400">
                        <span className="text-slate-200">CRM</span>
