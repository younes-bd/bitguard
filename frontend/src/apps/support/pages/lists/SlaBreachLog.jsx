...
const SlaBreachLog = () => {
    ...
    useEffect(() => {
        supportService.getSlaBreaches()
            .then(data => { setBreaches(Array.isArray(data) ? data : data.results || []); setLoading(false); })
            .catch(() => setLoading(false));
    }, []);

    const handleAcknowledge = async (id) => {
        try {
            await supportService.acknowledgeBreach(id);
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            <tr key={b.id} className="border-b border-slate-800/60 hover:bg-slate-800/30 transition-colors">
                                <td className="px-5 py-4 text-white font-medium">{b.contract?.client ?? b.contract ?? 'â€”'}</td>
                                <td className="px-5 py-4">{severityBadge(b.breach_type)}</td>
                                <td className="px-5 py-4 text-slate-400">{b.breached_at?.split('T')[0] ?? 'â€”'}</td>
                                <td className="px-5 py-4">
                                    {b.acknowledged
                                        ? <span className="text-emerald-400 text-xs font-semibold">âœ“ Acknowledged</span>
