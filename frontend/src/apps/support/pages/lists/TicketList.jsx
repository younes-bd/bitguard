                    ...
                    <tbody>
                        {loading ? (
                            ...
                            <tr key={t.id} className={`border-b transition-colors cursor-pointer ${isOverdue ? 'border-red-500/50 bg-red-900/10 hover:bg-red-900/20' : 'border-slate-800/60 hover:bg-slate-800/30'}`}>
                                ...
                                <td className="px-5 py-4 text-slate-400">{t.client?.name ?? t.client ?? 'â€”'}</td>
                                <td className="px-5 py-4">{priorityBadge(t.priority)}</td>
                                <td className="px-5 py-4">{statusBadge(t.status)}</td>
                                <td className="px-5 py-4 text-slate-400">{t.created_at?.split('T')[0] ?? 'â€”'}</td>
                                <td className="px-5 py-4 text-slate-400">{t.due_date ? t.due_date.split('T')[0] : 'None'}</td>
                                <td className="px-5 py-4">
                                    <button onClick={() => navigate(`/admin/support/tickets/${t.id}`)} className="text-blue-400 hover:text-blue-300 transition-colors">
                                        View
                                    </button>
                                </td>
                            </tr>
