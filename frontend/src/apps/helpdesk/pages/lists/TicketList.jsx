...
const priorityBadge = (priority) => {
    ...
    const isAiAssigned = priority === 'high' || priority === 'critical';
    return (
        <span className={`text-xs font-bold capitalize flex items-center gap-1 ${map[priority] ?? 'text-slate-400'}`}>
            {priority}
            {isAiAssigned && <Sparkles size={12} className="text-blue-400" title="Categorized by BitGuard AI" />}
        </span>
    );
};
    ...
    return (
        <div className="p-6 lg:p-8 space-y-6 animate-in fade-in duration-400">
            <div className="flex justify-between items-center">
                ...
                <button onClick={() => navigate('/admin/helpdesk/tickets/create')} className="flex items-center gap-2 bg-teal-600 hover:bg-teal-500 text-white px-4 py-2 rounded-lg text-sm font-semibold transition-colors">
                ...
                <table className="w-full text-sm">
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            return (
                            <tr key={t.id} className={`border-b transition-colors cursor-pointer ${isOverdue ? 'border-red-500/50 bg-red-900/10 hover:bg-red-900/20' : 'border-slate-800/60 hover:bg-slate-800/30'}`}>
                                ...
                                <td className="px-5 py-4">
                                    <button onClick={() => navigate(`/admin/helpdesk/tickets/${t.id}`)} className="text-blue-400 hover:text-blue-300 transition-colors">
                                        View
                                    </button>
                                </td>
                            </tr>
