...
export default function ITServiceCatalog() {
    ...
    const fetchServices = async () => {
        setLoading(true);
        try {
            const categoriesData = await serviceService.getServiceCategories();
            setCategories([{ id: 'all', slug: 'all', name: 'All Services', icon: 'Tag' }, ...categoriesData]);
            
            const data = await serviceService.getServiceItems();
            setServices(data);
        } catch (err) {
            console.error(err);
    ...
    // Categories are now dynamic from API
    ...
    const getIconComponent = (iconName) => {
        switch (iconName) {
            ...
            case 'Shield': return <ShieldCheck className="text-red-400 w-6 h-6" />;
            case 'Cpu': return <Cpu className="text-emerald-400 w-6 h-6" />;
            case 'Cloud': return <Cpu className="text-sky-400 w-6 h-6" />;
            case 'Monitor': return <Monitor className="text-blue-400 w-6 h-6" />;
            case 'Camera': return <Monitor className="text-slate-400 w-6 h-6" />;
            case 'Globe': return <Monitor className="text-purple-400 w-6 h-6" />;
        ...
        <div className="space-y-8 max-w-7xl mx-auto pb-12 px-4 sm:px-6">
            ...
            <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="md:w-64 shrink-0 flex flex-row md:flex-col gap-2 overflow-x-auto pb-2 md:pb-0 scrollbar-none">
                    {categories.map((cat) => {
                        const Icon = cat.icon === 'Tag' ? Tag : HelpCircle; // Simplified icon mapping for sidebar for now
                        const active = selectedCategory === (cat.slug || cat.id);
                        return (
                            <button
                                key={cat.slug || cat.id}
