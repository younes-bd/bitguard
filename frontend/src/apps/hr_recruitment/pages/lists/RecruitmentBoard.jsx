...
import { recruitmentService } from '@/apps/hr_recruitment/api/recruitmentService';
...
export default function RecruitmentBoard() {
    ...
    const loadData = () => {
        setLoading(true);
        Promise.all([
            recruitmentService.getJobPositions(),
            recruitmentService.getJobApplications()
    ...
    const handleHire = async (id) => {
        try {
            await recruitmentService.hireApplication(id);
                                    ...
                                    <tr key={app.id} className="hover:bg-slate-800/30">
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex justify-end gap-2">
                                                <button className="text-slate-400 hover:text-white px-3 py-1 rounded border border-slate-700 hover:bg-slate-800 transition-colors">Review</button>
                                                {app.stage !== 'hired' && (
                                                    <button 
                                                        onClick={() => handleHire(app.id)}
                                                        className="text-emerald-400 hover:text-white px-3 py-1 rounded border border-emerald-500/30 hover:bg-emerald-600 transition-colors"
                                                    >
                                                        Hire
                                                    </button>
                                                )}
