...
const RentalDashboard = () => {
  ...
  useEffect(() => {
    rentalService.getOrders().then(data => {
      setOrders(Array.isArray(data) ? data : data.results || []);
      setLoading(false);
    }).catch(() => setLoading(false));
  }, []);

  const activeRentals = orders.filter(o => o.state === 'sale').length;
  const quotes = orders.filter(o => o.state === 'draft' || o.state === 'sent').length;
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group overflow-hidden relative">
          ...
          <h3 className="text-slate-500 dark:text-slate-400 font-medium mb-1">Total Orders</h3>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group overflow-hidden relative">
          ...
          <h3 className="text-slate-500 dark:text-slate-400 font-medium mb-1">Active Rentals</h3>
          <p className="text-3xl font-bold text-slate-800 dark:text-white">{loading ? '...' : activeRentals}</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group overflow-hidden relative">
          ...
          <h3 className="text-slate-500 dark:text-slate-400 font-medium mb-1">Quotations</h3>
          <p className="text-3xl font-bold text-slate-800 dark:text-white">{loading ? '...' : quotes}</p>
        </div>
      </div>
