...
export default function DocumentDetail() {
    ...
    const fetchDocument = async () => {
        setLoading(true);
        try {
            const res = await documentsService.getDocument(id);
            setDoc(res);
        } catch (err) {
    ...
    const handleAction = async (actionStr) => {
        try {
            if (actionStr === 'archive') {
                ...
                await documentsService.lockDocument(id);
                fetchDocument();
            } else if (actionStr === 'share') {
                const res = await documentsService.shareDocument(id);
        ...
        <div className="space-y-6 max-w-7xl mx-auto animate-in fade-in duration-500 pb-10">
            ...
            <div className="flex justify-between items-start bg-slate-900 p-6 rounded-2xl border border-slate-800">
                <div className="space-y-4">
                    <Link to="/admin/documents" className="text-slate-400 hover:text-cyan-400 text-sm font-medium flex items-center gap-2 w-fit transition-colors">
