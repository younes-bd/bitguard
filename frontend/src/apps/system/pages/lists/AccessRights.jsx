...
export default function AccessRights() {
        ...
        const loadData = async () => {
            ...
            try {
                const [rolesRes, ctRes, matrixRes] = await Promise.all([
                    ...
                    client.get('users/content-types/'),
                    client.get('users/role-permissions/matrix/')
                ]);
    ...
    const togglePermission = async (roleId, ctId, permType, currentVal) => {
        ...
        try {
            ...
            const existingRes = await client.get(`users/role-permissions/?role=${roleId}&content_type=${ctId}`);
            let existing = null;
            ...
            if (existing) {
                await client.patch(`users/role-permissions/${existing.id}/`, payload);
            } else {
                await client.post(`users/role-permissions/`, payload);
            }
        } catch (err) {
