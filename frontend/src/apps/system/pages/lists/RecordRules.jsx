...
const RecordRules = () => {
    ...
    const fetchRules = async () => {
        try {
            setLoading(true);
            const data = await usersService.getRecordRules();
    ...
    useEffect(() => {
        fetchRules();
        client.get('users/roles/').then(r => setRoles(r.data?.data?.roles || r.data?.data || r.data?.results || r.data || []));
        client.get('users/content-types/').then(r => {
    ...
    const handleSave = async () => {
        try {
            if (editingRule) {
                await usersService.updateRecordRule(editingRule.id, form);
                toast.success('Record rule updated');
            } else {
                await usersService.createRecordRule(form);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await usersService.deleteRecordRule(id);
