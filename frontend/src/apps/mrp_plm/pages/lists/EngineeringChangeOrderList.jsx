...
const EngineeringChangeOrderList = () => {
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure?")) {
      plmService.deleteECO(id).then(() => fetchData());
    }
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button 
          onClick={() => navigate(`/admin/mrp_plm/engineeringchangeorders/new`)}
              ...
              {loading ? (
                ...
                <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right space-x-2">
                    <button 
                      onClick={() => navigate(`/admin/mrp_plm/engineeringchangeorders/${item.id}`)}
...
export default EngineeringChangeOrderList;
