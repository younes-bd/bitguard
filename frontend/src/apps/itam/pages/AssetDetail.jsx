...
import { Monitor, ArrowLeft, Clock, AlertTriangle, CheckCircle, Wrench, Archive } from 'lucide-react';
...
export default function AssetDetail() {
    ...
    useEffect(() => {
        Promise.all([
            client.get(`itam/assets/${id}/`),
            client.get(`itam/assets/${id}/history/`).catch(() => ({ data: [] })),
    ...
    const handleAction = async (action) => {
        ...
        try {
            ...
            await client.patch(`itam/assets/${id}/`, { status });
    ...
    if (!asset) return (
        <div className="text-center py-20 text-slate-500">
            ...
            <button onClick={() => navigate('/admin/itam/assets')} className="mt-4 text-teal-400 text-sm hover:underline">Back to Assets</button>
        </div>
    );
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6 pb-12 px-4 sm:px-6 animate-in fade-in duration-500">
            <div className="flex items-center gap-4">
                <button onClick={() => navigate('/admin/itam/assets')} className="p-2 bg-slate-800 text-slate-400 hover:text-white rounded-xl transition-colors">
                    <ArrowLeft size={20} />
                </button>
                            ...
                            {history.map((h, i) => (
                                <div key={i} className="flex items-start gap-3 pl-2">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-slate-500 text-xs">{h.user || h.assigned_to} · {h.date || h.created_at}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
