...
const QuoteTemplateList = () => {
  ...
  useEffect(() => {
    salesService.getQuotationTemplates().then(data => {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight flex items-center gap-3">
            <FileText className="text-purple-400" size={30} /> Quotation Templates
          </h1>
          <p className="text-slate-400 mt-1">Manage reusable quotation formats.</p>
        </div>
        <button
        ...
        {loading ? (
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-800/50">
              {filtered.length === 0 ? (
                <tr><td colSpan="2" className="p-12 text-center text-slate-500">No templates found.</td></tr>
              ) : filtered.map(t => (
