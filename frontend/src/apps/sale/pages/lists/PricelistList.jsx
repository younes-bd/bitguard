...
const PricelistList = () => {
  ...
  useEffect(() => {
    salesService.getPricelists().then(data => {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight flex items-center gap-3">
            <DollarSign className="text-purple-400" size={30} /> Pricelists
          </h1>
          <p className="text-slate-400 mt-1">Manage product pricing rules.</p>
        </div>
        <button
        ...
        {loading ? (
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-800/50">
              {filtered.length === 0 ? (
                <tr><td colSpan="3" className="p-12 text-center text-slate-500">No pricelists found.</td></tr>
              ) : filtered.map(p => (
                <tr key={p.id} className="hover:bg-slate-800/30 transition-colors">
                  <td className="p-4 font-bold text-white">{p.name}</td>
                  <td className="p-4 font-mono text-slate-300">{p.currency}</td>
                  <td className="p-4">
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold border uppercase tracking-widest ${p.active ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : 'bg-red-500/20 text-red-400 border-red-500/30'}`}>
