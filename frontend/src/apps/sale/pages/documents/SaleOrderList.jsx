...
const SaleOrderList = () => {
  ...
  useEffect(() => {
    salesService.getSaleOrders({ status: 'sale' }).then(data => {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight flex items-center gap-3">
            <FileCheck className="text-purple-400" size={30} /> Sale Orders
          </h1>
          <p className="text-slate-400 mt-1">Manage customer orders and their status.</p>
        </div>
        <button
          onClick={() => navigate('/admin/sales/orders/create')}
        ...
        {loading ? (
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-800/50">
              {filtered.length === 0 ? (
                <tr><td colSpan="6" className="p-12 text-center text-slate-500">No sale orders yet. Create your first one!</td></tr>
              ) : filtered.map(q => (
