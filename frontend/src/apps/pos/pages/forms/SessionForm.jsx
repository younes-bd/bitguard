...
const SessionForm = () => {
  ...
  useEffect(() => {
    if (isEdit) {
      posService.getSession(id).then(data => {
        setFormData(data);
      });
    }
  }, [id, isEdit]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const action = isEdit ? posService.updateSession(id, formData) : posService.createSession(formData);
    action.then(() => {
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm space-y-6">
        <div className="grid grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Config ID</label>
            <input 
              type="number"
              name="config"
          ...
          <div className="space-y-2">
            ...
            <select 
              ...
              className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-indigo-500/20"
            >
              <option value="opening_control">Opening Control</option>
              <option value="opened">In Progress</option>
              <option value="closing_control">Closing Control</option>
