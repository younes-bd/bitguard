...
export default function ItsmDashboard() {
    ...
    const handleAction = async (actionFn, successMsg) => {
        ...
        try {
            ...
            const updated = await itsmService.getChangeRequest(selectedChange.id);
        ...
        <div className="space-y-8 max-w-7xl mx-auto pb-12 px-4 sm:px-6">
                        ...
                        <div className="p-6 bg-slate-950 border-t border-slate-800 flex items-center justify-between">
                            <div className="flex gap-3">
                                {selectedChange.status === 'draft' && (
                                    <button 
                                        onClick={() => handleAction(itsmService.submit, 'Change submitted for review')}
                                ...
                                {selectedChange.status === 'submitted' && (
                                    <>
                                        <button 
                                            onClick={() => handleAction(itsmService.approve, 'Change approved by CAB')}
                                            disabled={actionLoading}
                                        ...
                                        <button 
                                            onClick={() => handleAction(itsmService.reject, 'Change request rejected')}
                                            disabled={actionLoading}
                                ...
                                {selectedChange.status === 'approved' && (
                                    <button 
                                        onClick={() => handleAction(itsmService.startWork, 'Implementation started')}
                                        disabled={actionLoading}
                                ...
                                {selectedChange.status === 'in_progress' && (
                                    <>
                                        <button 
                                            onClick={() => handleAction(itsmService.complete, 'Change successfully completed')}
                                            disabled={actionLoading}
                                            className="bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 transition-all"
                                        >
                                        ...
                                        <button 
                                            onClick={() => handleAction(itsmService.fail, 'Change marked as failed')}
                                            disabled={actionLoading}
                                            className="bg-rose-600 hover:bg-rose-500 text-white px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 transition-all"
                                        >
                                ...
                                {selectedChange.status === 'failed' && (
                                    <button 
                                        onClick={() => handleAction(itsmService.rollback, 'Rollback protocol initiated')}
                                        disabled={actionLoading}
            ...
            <GenericModal
                ...
                onSubmit={async (formData) => {
                    setActionLoading(true);
                    try {
                        await itsmService.createChangeRequest(formData);
