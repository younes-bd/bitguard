...
import { GitBranch, Plus, Search, CheckCircle, AlertCircle, Clock, Play, X, ChevronRight, Loader2, XCircle, RotateCcw, CheckSquare, User, Calendar } from 'lucide-react';
...
export default function ChangeRequestList() {
    ...
    const fetchChanges = async () => {
        setLoading(true);
        try {
            const data = await itsmService.getChangeRequests();
    ...
    const fetchTasks = async (changeId) => {
        ...
        try {
            const data = await itsmService.getTasks(changeId);
    ...
    const handleCreateTask = async (formData) => {
        setActionLoading(true);
        try {
            await itsmService.createTask({ ...formData, change_request: selected.id });
    ...
    const handleCompleteTask = async (taskId) => {
        try {
            await itsmService.updateTask(taskId, { status: 'completed' });
