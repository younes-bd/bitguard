        ...
        <div className="p-6 lg:p-8 space-y-6 animate-in fade-in duration-400">
            ...
            <div className="space-y-2">
                {loading ? (
                    ...
                    notifications.map(n => {
                        const isSlaBreach = (n.title || '').includes('SLA Breach');
                        return (
                        <div key={n.id}
                            onClick={() => !n.is_read && markRead(n.id)}
                            className={`flex items-start gap-4 p-4 rounded-xl border transition-colors cursor-pointer
                                ${isSlaBreach && !n.is_read ? 'bg-red-500/5 border-red-500/50 animate-pulse' : 
                                  n.is_read ? 'bg-slate-900/40 border-slate-800/60' : 'bg-slate-900 border-slate-700 hover:border-blue-500/30'}`}>
                            <div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${isSlaBreach ? 'bg-red-500' : n.is_read ? 'bg-slate-700' : 'bg-blue-500'}`} />
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1">
                                    {isSlaBreach ? (
                                        <span className="bg-red-500/20 text-red-400 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase flex items-center gap-1">
                                            <Bell size={10} /> SLA BREACH
                                        </span>
                                    ) : typeBadge(n.notification_type ?? n.type)}
                                    <span className="text-slate-500 text-xs">{timeAgo(n.created_at)}</span>
                                </div>
                                <p className={`text-sm leading-snug ${isSlaBreach ? 'text-red-200' : n.is_read ? 'text-slate-400' : 'text-slate-200 font-medium'}`}>
                                    {n.message}
                                </p>
                                {n.url && (
                                    <a href={n.url} className="text-blue-400 text-xs mt-1 hover:text-blue-300 inline-block no-underline">View details â†’</a>
                                )}
                            </div>
                        </div>
                    ); })
