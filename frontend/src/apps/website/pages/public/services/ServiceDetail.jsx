...
const ServiceDetail = () => {
    ...
    useEffect(() => {
        const fetchService = async () => {
            setLoading(true);
            try {
                const data = await websiteService.getServiceBySlug(slug);
                setServiceData(data);
                
                const all = await websiteService.getServices();
                setAllServices(all);
            } catch (err) {
                setError(err);
            } finally {
    ...
    if (loading) {
        return <div className="min-h-screen flex items-center justify-center dark:bg-slate-950 bg-slate-50"><div className="w-10 h-10 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div></div>;
    }
    ...
    const contentData = typeof serviceData.content_data === 'string' ? JSON.parse(serviceData.content_data) : (serviceData.content_data || {});
    const service = { ...serviceData, ...contentData };
    ...
    // M-15: Basic sanitization for dangerouslySetInnerHTML
    const sanitizeHTML = (html) => {
        if (!html) return '';
        // Very basic XSS mitigation since DOMPurify is missing
    ...
    const colors = ['bg-red-500/10 text-red-500', 'bg-blue-500/10 text-blue-500', 'bg-violet-500/10 text-violet-500'];
    
    // H-13: Use specific related services if defined, else pick deterministically based on slug length to avoid same 3
    let relatedKeys = service.relatedServices || [];
    if (!relatedKeys || relatedKeys.length === 0) {
        const allKeys = allServices.filter(s => s.slug !== slug).map(s => s.slug);
        const shift = slug.length % (allKeys.length || 1);
        relatedKeys = [...allKeys.slice(shift), ...allKeys.slice(0, shift)].slice(0, 3);
    }

    const relatedServices = relatedKeys.map((key, i) => {
        const relService = allServices.find(s => s.slug === key);
        return {
            to: `/solutions/${key}`,
            icon: relService?.icon || 'bi bi-star',
            color: colors[i % colors.length],
            label: relService?.title || key,
            sub: relService?.description || ''
        };
    });
    ...
    return (
        <div className="dark:bg-slate-950 bg-slate-50 min-h-screen font-sans selection:bg-sky-500/30 transition-colors duration-300">
                ...
                <div className="flex flex-col lg:flex-row gap-16">
                    ...
                    <div className="flex-1 space-y-32 min-w-0">
                        ...
                        <section id="overview" className="scroll-mt-32">
                            <div className="dark:prose-invert prose prose-xl prose-slate max-w-none dark:text-slate-300 text-slate-600 leading-relaxed font-light marker:text-blue-500 prose-p:mb-6 prose-strong:text-slate-900 dark:prose-strong:text-white" dangerouslySetInnerHTML={{ __html: sanitizeHTML(service.long_description) }}></div>
                        </section>
            ...
            {/* RELATED SERVICES */}
            <div className="dark:bg-slate-900 bg-slate-50 border-t dark:border-slate-800 border-slate-200">
                <div className="container mx-auto px-4 md:px-8 py-16">
                    <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-6">You May Also Need</p>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                        {relatedServices.map(item => (
                            <Link key={item.to} to={item.to} className="flex items-start gap-4 p-6 rounded-2xl dark:bg-slate-800/50 bg-white border dark:border-slate-700 border-slate-200 hover:shadow-lg hover:-translate-y-0.5 transition-all no-underline group">
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${item.color} text-lg`}>
                                    <i className={item.icon}></i>
                                </div>
                                <div>
                                    <div className="dark:text-white text-slate-900 font-bold text-sm mb-1 group-hover:text-blue-500 transition-colors">{item.label}</div>
                                    <div className="text-slate-500 text-[13px] leading-relaxed">{item.sub}</div>
                                </div>
                            </Link>
                        ))}
                    </div>
                    <div className="mt-8 text-center">
                        <Link to="/contact" className="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold text-sm no-underline transition-all shadow-lg shadow-blue-600/20">
