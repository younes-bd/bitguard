...
const Brochure = () => {
    ...
    useEffect(() => {
        const fetchService = async () => {
            try {
                ...
                setServiceData(data);
                document.title = `${data.title} - Brochure`;
            } catch (err) {
                setError(err);
            } finally {
                setLoading(false);
    ...
    if (loading) {
        return <div className="min-h-screen flex items-center justify-center bg-slate-100"><div className="w-8 h-8 border-4 border-sky-600 border-t-transparent rounded-full animate-spin"></div></div>;
    }
                ...
                <div className="flex justify-between items-end border-b-4 border-sky-600 pb-8 mb-12 flex-shrink-0">
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-[12px]">
                            ...
                            <div>
                                <span className="text-slate-900 text-[26px] font-[Oswald] font-semibold tracking-[2px] leading-none block scale-y-[1.3] origin-left uppercase drop-shadow-none group-hover:drop-shadow-[0_0_12px_rgba(56,189,248,0.8)] transition-all duration-300">BITGUARD</span>
