...
const Reports = () => {
    ...
    const handleSubscribe = async (e) => {
        ...
        try {
            await client.post('home/signups/', { email: subscribeEmail });
    ...
    const handleDownload = (e) => {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                // Trigger real download silently
                const pdfContent = '%PDF-1.4\n1 0 obj\n<< /Title (BitGuard Report) >>\nendobj\ntrailer\n<< /Root 1 0 R >>\n%%EOF';
                const blob = new Blob([pdfContent], { type: 'application/pdf' });
                const url = URL.createObjectURL(blob);
                const link = document.createElement('a');
                link.href = url;
                link.download = `bitguard-report-${selectedReport?.id || 'sample'}.pdf`;
    ...
    const reports = [
        ...
        },
        {
            ...
            image: "/assets/images/home/dashboards.png",
            pages: 25
        },
        {
            id: 4,
        ...
        <div className="dark:bg-slate-950 bg-slate-50 min-h-screen transition-colors duration-300">
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-12">
                        ...
                        <div className="md:w-1/2 relative">
                            ...
                            <div className="dark:bg-slate-800 bg-white rounded-xl shadow-2xl p-6 md:p-8 relative z-10 transform md:rotate-2 hover:rotate-0 transition-transform duration-500">
                                ...
                                <button onClick={() => setSelectedReport(reports[0])} className="w-full py-3 dark:bg-blue-600 bg-slate-900 text-white font-bold rounded-lg dark:hover:bg-blue-500 hover:bg-blue-600 transition-colors duration-300">
                ...
                <div className="container mx-auto px-4">
                    <div className="flex items-center justify-between mb-10">
                        ...
                        <div className="hidden md:flex gap-2">
                            {['All', 'Annual Report', 'Whitepaper', 'Case Study', 'Guide'].map((filter, i) => (
                                <button 
                                    key={i} 
                                    onClick={() => setActiveFilter(filter)}
                                    className={`px-4 py-2 rounded-full text-sm font-bold transition-colors duration-300 ${activeFilter === filter ? 'dark:bg-blue-600 bg-slate-900 text-white' : 'dark:bg-slate-800 bg-white dark:text-slate-300 text-slate-600 dark:hover:bg-slate-700 hover:bg-slate-100'}`}>
                                    {filter}
                                </button>
                            ))}
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {(activeFilter === 'All' ? reports : reports.filter(r => r.category === activeFilter)).map((report) => (
            ...
            <section className="py-20 dark:bg-slate-950 bg-slate-950 text-white text-center relative overflow-hidden transition-colors duration-300">
                ...
                <div className="container mx-auto px-4 relative z-10">
                    ...
                    <form onSubmit={handleSubscribe} className="max-w-md mx-auto relative flex flex-col sm:flex-row gap-2">
                        <input 
                            type="email" 
                            required 
                            ...
                            className="flex-1 px-4 py-4 rounded-xl border border-slate-700 bg-slate-900 text-white focus:outline-none focus:border-blue-500" 
                        />
                        <button 
                            type="submit" 
                            disabled={isSubscribing}
                            className="bg-blue-600 text-white px-8 py-4 rounded-xl font-bold uppercase tracking-wider hover:bg-blue-500 transition-colors shadow-xl shadow-blue-600/30 disabled:opacity-70"
                        >
                            {isSubscribing ? 'Subscribing...' : 'Subscribe'}
                        </button>
                    </form>
                    {subscribeMessage && <p className="mt-4 text-sm font-bold text-emerald-400">{subscribeMessage}</p>}
                </div>
            </section>
            ...
            {selectedReport && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm transition-opacity">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden relative" onClick={e => e.stopPropagation()}>
                        <button 
                            onClick={() => { setSelectedReport(null); setIsSuccess(false); }}
                            className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 cursor-pointer border-none bg-transparent"
                        >
                        ...
                        <div className="p-8">
                            <div className="text-center mb-6">
                                <div className="w-16 h-16 bg-blue-500/10 text-blue-600 dark:text-blue-400 rounded-full flex items-center justify-center text-3xl mx-auto mb-4">
                                    <i className="bi bi-file-earmark-pdf-fill"></i>
                                </div>
                                <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Download Report</h3>
                                <p className="text-slate-500 text-sm">{selectedReport.title}</p>
                            </div>
                            ...
                            {isSuccess ? (
                                <div className="text-center py-8 animate-fade-in">
                                    ...
                                    <p className="text-slate-500 text-sm">Your download is starting automatically.</p>
                                </div>
                            ) : (
                                <form onSubmit={handleDownload} className="space-y-4">
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Work Email <span className="text-red-500">*</span></label>
                                        <input type="email" required placeholder="john@company.com" className="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">First Name</label>
                                            <input type="text" required placeholder="John" className="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Last Name</label>
                                            <input type="text" required placeholder="Doe" className="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                        </div>
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Company Name</label>
                                        <input type="text" required placeholder="Company Inc." className="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                    </div>
                                    
                                    <button 
                                        type="submit" 
                                        disabled={isSubmitting}
                                        className="w-full py-4 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-lg shadow-lg shadow-blue-500/20 transition-all disabled:opacity-70 flex items-center justify-center mt-6"
                                    >
                                        {isSubmitting ? (
                                    ...
                                    <p className="text-xs text-center text-slate-400 mt-4">
                                        By downloading, you agree to our <Link to="/privacy" className="text-blue-500 hover:underline">Privacy Policy</Link>.
                                    </p>
                                </form>
                            )}
                        </div>
