    ...
    const handlePublishToggle = async (page) => {
        try {
            if (page.status === 'published') {
                await cmsService.unpublishPage(page.slug);
            } else {
                await cmsService.publishPage(page.slug);
            }
            loadPages();
        } catch (err) {
                        ...
                        {pages.length === 0 ? (
                            ...
                            pages.map((page) => (
                                <tr key={page.id} className="hover:bg-slate-800/50 transition-colors group">
                                    ...
                                    <td className="p-4">
                                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${page.status === 'published' ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'}`}>
                                    ...
                                    <td className="p-4 text-right">
                                        <div className="flex items-center justify-end gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                                            <button
                                                ...
                                                className={`p-2 rounded-lg transition-colors ${page.status === 'published' ? 'text-amber-400 hover:bg-amber-500/10' : 'text-green-400 hover:bg-green-500/10'}`}
                                                title={page.status === 'published' ? 'Unpublish' : 'Publish'}
                                            >
                                                {page.status === 'published' ? <EyeOff size={16} /> : <Eye size={16} />}
                                            </button>
                                            <Link
                                                to={`/admin/website/pages/${page.slug}/edit`}
