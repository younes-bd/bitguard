        ...
        const loadData = async () => {
            try {
                const [stats, logs] = await Promise.all([
                    ...
                    iamService.getAuditLogs({ limit: 5 })
                ]);
        ...
        <div className="p-6 lg:p-8 space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex gap-3">
                    <Link to="/admin/iam/audit" className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl text-sm font-semibold transition-all border border-slate-700">
                        Audit Logs
                    </Link>
                    <Link to="/admin/iam/users" className="px-5 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-xl text-sm font-bold transition-all shadow-lg shadow-purple-600/20">
                        Provision User
                    </Link>
                </div>
            </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-slate-900/50 border border-slate-800 rounded-3xl overflow-hidden backdrop-blur-xl">
                        <div className="px-6 py-5 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
                            ...
                            <Link to="/admin/iam/audit" className="text-[10px] font-black text-purple-400 uppercase hover:text-purple-300 tracking-tighter bg-purple-400/10 px-2 py-1 rounded">View All Vectors</Link>
                        </div>
...
const MetricCard = ({ label, value, icon: Icon, color, subText }) => (
    <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-6 relative overflow-hidden group hover:border-slate-700 transition-all">
        ...
        <div className="space-y-1">
            ...
            <p className="text-3xl font-extrabold text-white tracking-tighter">{value.toLocaleString()}</p>
