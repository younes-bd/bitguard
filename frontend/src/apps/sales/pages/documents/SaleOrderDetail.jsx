...
const SaleOrderDetail = () => {
  ...
  const fetchOrder = async () => {
    try {
      const data = await erpService.getSaleOrder(id);
      setOrder(data);
    } catch (e) {
      console.error(e);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {(order.status === 'draft' || order.status === 'sent') && (
            <button
              onClick={() => handleAction(() => erpService.confirmSaleOrder(id), 'Order Confirmed')}
          ...
          {order.status === 'sale' && (
            <>
              <button
                onClick={() => handleAction(() => erpService.invoiceSaleOrder(id), 'Invoice Created')}
                disabled={actionLoading}
              ...
              <button
                onClick={() => handleAction(() => erpService.createDeliveryFromSale(id), 'Delivery Note Created')}
                disabled={actionLoading}
          ...
          {order.status !== 'cancel' && order.status !== 'done' && (
            <button
              onClick={() => handleAction(() => erpService.cancelSaleOrder(id), 'Order Cancelled')}
              disabled={actionLoading}
