...
const VendorList = () => {
    ...
    useEffect(() => {
        purchaseService.getVendors({ search }).then(d => {
    ...
    const handleSave = async (formData) => {
        ...
        try {
            if (selectedVendor) {
                await purchaseService.updateVendor(selectedVendor.id, formData);
            } else {
                await purchaseService.createVendor(formData);
            }
            setIsModalOpen(false);
            const data = await purchaseService.getVendors({ search });
