...
const EcommerceDashboard = () => {
    ...
    const loadDashboardData = async () => {
        try {
            ...
            const [ordersRes, licensesRes, productsRes] = await Promise.all([
                ecommerceService.getOrders(),
                ecommerceService.getLicenses(),
            ...
            setAddons(products.slice(0, 3)); // Show top 3 products as addons
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="glass-panel p-6 rounded-xl border border-slate-700/50 bg-gradient-to-br from-blue-900/10 to-slate-900/50">
                    ...
                    <div className="text-sm text-slate-400 font-medium uppercase">Total Products</div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="glass-panel p-6 rounded-xl border border-slate-700/50 bg-gradient-to-b from-slate-800/40 to-slate-900/40">
                    ...
                    <div className="space-y-4">
                        {addons.length > 0 ? addons.map((product) => (
                            <div key={product.id} className="group p-4 rounded-lg bg-slate-900/50 border border-slate-700/50 hover:border-indigo-500/50 transition-all cursor-pointer">
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    {product.is_featured && <span className="text-xs font-bold text-emerald-400">Featured</span>}
                                </div>
                                <h3 className="font-bold text-white mb-1 truncate">{product.name}</h3>
                                <p className="text-xs text-slate-500 mb-3 truncate">{product.description || 'Upgrade your setup with this product.'}</p>
                                <div className="flex items-center justify-between text-sm">
                                    <span className="text-white font-bold">${product.price || product.base_price || '0.00'}</span>
                                    <ChevronRight size={16} className="text-slate-500 group-hover:text-indigo-400 transition-colors" />
                                </div>
                            </div>
                        )) : (
                            <p className="text-sm text-slate-500 text-center py-4">No addons available.</p>
                        )}
                    </div>

                    <button onClick={() => navigate('/admin/store/products')} className="w-full mt-6 py-3 rounded-lg border border-slate-700 text-slate-300 hover:text-white hover:bg-slate-800 transition-colors text-sm font-medium">
                        View All Offers
                    </button>
                </div>
            </div>
