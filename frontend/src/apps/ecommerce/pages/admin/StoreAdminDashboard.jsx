...
const StoreAdminDashboard = () => {
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                const [stats, mrrData, ordersRes] = await Promise.all([
                    boardService.getStats(),
                    boardService.getMrrHistory(),
                    ecommerceService.getOrders()
                ]);
                
                setMetrics(stats);
                if (mrrData?.monthly_history) {
                    setChartData(mrrData.monthly_history);
                }
