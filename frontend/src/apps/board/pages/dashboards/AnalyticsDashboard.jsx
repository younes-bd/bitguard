...
const AnalyticsDashboard = () => {
    ...
    useEffect(() => {
        ...
        boardService.getMetrics().then(res => {
            const data = res.data ?? {};
            setSummary({
                monthlyRevenue: data.billing?.mrr,
                winRate: data.crm?.funnel?.win_rate,
                openTickets: data.support?.open_tickets,
                openAlerts: data.security?.open_alerts,
                activeUsers: data.hr?.total_employees
            });
        }).catch(() => {
            setSummary({});
        });
    ...
    const tiles = [
        { title: 'Revenue Analytics', desc: 'Financial health, MRR growth, and collection velocity', path: '/admin/board/revenue', icon: TrendingUp, color: 'emerald', value: summary.monthlyRevenue != null ? `$${Number(summary.monthlyRevenue).toLocaleString()}` : undefined },
        { title: 'Finance & P&L', desc: 'Profit, loss, expenses, and A/R aging', path: '/admin/board/finance', icon: DollarSign, color: 'emerald' },
        { title: 'Sales Performance', desc: 'Lead-to-deal funnel, win rate, and pipeline health', path: '/admin/board/crm', icon: Users, color: 'blue', value: summary.winRate != null ? `${summary.winRate}% win rate` : undefined },
        { title: 'Service Desk Ops', desc: 'Ticket volume, SLA compliance, and support efficiency', path: '/admin/board/support', icon: LifeBuoy, color: 'amber', value: summary.openTickets != null ? `${summary.openTickets} open` : undefined },
        { title: 'Security Posture', desc: 'Threat vectors, incident response, and risk mitigation', path: '/admin/board/security', icon: ShieldAlert, color: 'rose', value: summary.openAlerts != null ? `${summary.openAlerts} active alerts` : undefined },
        { title: 'People & HR', desc: 'Headcount, certifications, and time utilization', path: '/admin/board/hrm', icon: Users, color: 'blue' },
        { title: 'Project Management', desc: 'Active projects, budgets vs actuals', path: '/admin/board/projects', icon: Activity, color: 'amber' },
    ];
