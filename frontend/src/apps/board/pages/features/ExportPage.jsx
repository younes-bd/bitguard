...
const EXPORT_TYPES = [
    ...
    { key: 'projects', label: 'Projects & Tasks', description: 'Project status, logged hours, and task completion', format: 'CSV' },
];
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            for (const key of selected) {
                const res = await boardService.exportReport(key);
                
                const exportType = EXPORT_TYPES.find(e => e.key === key);
                const format = exportType ? exportType.format.toLowerCase() : 'csv';
                
                const blob = res instanceof Blob ? res : new Blob([res], { type: format === 'pdf' ? 'application/pdf' : 'text/csv' });
                const url = window.URL.createObjectURL(blob);
                const link = document.createElement('a');
                link.href = url;
                link.setAttribute('download', `bitguard-${key}-export.${format}`);
