...
const CreditNoteCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/admin/accounting/credit-notes');
    } catch (err) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 pb-20 animate-in fade-in duration-500">
      <div className="flex items-center gap-4">
        <button onClick={() => navigate('/admin/accounting/credit-notes')} className="p-2 hover:bg-slate-800 rounded-lg text-slate-400 transition-colors border border-slate-700/30">
          <ArrowLeft size={20} />
        </button>
        <div>
      ...
      <form onSubmit={handleSubmit} className="glass-panel p-8 rounded-2xl border border-slate-700/50 space-y-6">
        ...
        <div className="flex justify-end gap-4 pt-6 border-t border-slate-800">
          <button type="button" onClick={() => navigate('/admin/accounting/credit-notes')} className="text-sm font-bold text-slate-500 hover:text-white transition-colors">Cancel</button>
