    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const clientRes = await client.post('/crm/clients/', {
                ...
                website: formData.domain ? `https://${formData.domain}` : '',
                status: 'managed_service'
            });
            const clientId = clientRes.data.id || clientRes.data.data?.id;

            // 2. Create Contact
            const [firstName, ...lastNames] = formData.contactName.split(' ');
            const contactRes = await client.post('/crm/contacts/', {
                client: clientId,
                first_name: firstName,
                last_name: lastNames.join(' ') || 'Contact',
                email: formData.contactEmail,
                phone: formData.contactPhone,
                role: formData.contactRole,
                is_primary: true
            });
            const contactId = contactRes.data.id || contactRes.data.data?.id;

            // 3. Create Lead (Auditable record of the start)
            const leadRes = await client.post('/crm/leads/', {
                title: `${formData.companyName} - Initial Onboarding`,
                contact: contactId,
                status: 'converted',
                value: 0.00
            });
            const leadId = leadRes.data.id || leadRes.data.data?.id;

            // 4. Create Deal (Record of the financial commitment)
            await client.post('/crm/deals/', {
                title: `${formData.companyName} ${formData.plan} Contract`,
                client: clientId,
                amount: formData.plan === 'enterprise' ? 50000.00 : (formData.plan === 'professional' ? 12000.00 : 5000.00),
                stage: 'won',
            ...
            await client.post('/crm/activities/', {
                client: clientId,
                activity_type: 'note',
                description: `Client provisioned via Onboarding Wizard. Plan: ${formData.plan}, Region: ${formData.region}`
            });

            setSuccess(true);
        } catch (err) {
            console.error("Onboarding Error:", err);
            setError(err.response?.data?.error || err.response?.data?.message || err.message || 'Onboarding failed.');
        } finally {
            setLoading(false);
        }
    };
