...
const AddQuoteLine = ({ quoteId, onAdd }) => {
    ...
    const [formData, setFormData] = useState({ product: '', description: '', quantity: 1, unit_price: 0 });
    ...
    const handleProductChange = (e) => {
        ...
        if (prod) {
            setFormData({ ...formData, product: prodId, description: prod.name, unit_price: prod.price });
        } else {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            await client.post(`contracts/quote-lines/`, {
                quote: quoteId,
            ...
            import('react-hot-toast').then(m => m.toast.success('Line item added'));
            setIsAdding(false);
            setFormData({ product: '', description: '', quantity: 1, unit_price: 0 });
            onAdd();
        } catch (error) {
            console.error(error);
            import('react-hot-toast').then(m => m.toast.error('Failed to add line item'));
        } finally {
            setLoading(false);
        }
    };
    ...
    if (!isAdding) {
        return (
            <button onClick={() => setIsAdding(true)} className="text-sm font-medium text-sky-400 hover:text-sky-300 flex items-center gap-1 transition-colors">
                <Plus size={16} /> Add Service Offer
            </button>
        );
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="md:col-span-2">
                    <label className="block text-xs font-medium text-slate-400 mb-1">Service Offer (Catalog)</label>
                    <select
                ...
                <div className="md:col-span-2 flex items-end gap-2">
                    <button type="submit" disabled={loading} className="px-4 py-2 bg-sky-600 hover:bg-sky-500 text-white rounded font-medium disabled:opacity-50 flex-1">
                        Save Line
                    </button>
                    <button type="button" onClick={() => setIsAdding(false)} className="px-4 py-2 border border-slate-700 text-slate-400 hover:text-white hover:bg-slate-800 rounded font-medium">
                        Cancel
                    </button>
                </div>
            </div>
...
const QuoteDetail = () => {
    ...
    const fetchQuote = async () => {
        try {
            const response = await client.get(`contracts/quotes/${id}/`);
    ...
    const handleAction = async (action) => {
        ...
        try {
            const response = await client.post(`contracts/quotes/${id}/${action}/`);
    ...
    return (
        <div className="space-y-6">
            {/* Back Button */}
            <button
                onClick={() => navigate('/admin/contracts/quotes')}
                        ...
                        <button 
                            onClick={async () => {
                                try {
                                    ...
                                    await reportingService.generateReport(null, 'contracts.Quote', id);
                    ...
                    <div className="glass-panel rounded-xl border border-slate-700/50 bg-slate-800/40 backdrop-blur-md overflow-hidden">
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-slate-700/50 text-slate-300">
                                {quote.lines && quote.lines.map((item) => (
                                    <tr key={item.id} className="hover:bg-slate-700/30">
                                        <td className="px-6 py-4">
                                            <div className="font-medium text-white">{item.product_name || item.description}</div>
                                            {item.product_name && <div className="text-xs text-slate-500">{item.description}</div>}
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-right font-medium text-emerald-400 font-mono">${parseFloat(item.line_total).toFixed(2)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                        ...
                        {isActionable && (
                            <div className="p-4 bg-slate-800/30 border-t border-slate-700">
                                <AddQuoteLine quoteId={id} onAdd={fetchQuote} />
                            </div>
                        )}
                    </div>
                </div>
