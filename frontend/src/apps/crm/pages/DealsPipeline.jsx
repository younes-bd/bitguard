...
const DealsPipeline = () => {
    ...
    const handleSaveDeal = async (formData) => {
        try {
            if (editingDeal) {
                const res = await crmService.updateDeal(editingDeal.id, formData);
                if (formData.stage === 'won' && editingDeal.stage !== 'won') {
                    // Trigger onboarding
                    setIsModalOpen(false);
                    navigate(`/admin/crm/onboarding?client=${res.client || editingDeal.client}`);
                    return;
                }
            } else {
