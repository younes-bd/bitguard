...
const SOURCE_MAP = {
    website: 'ðŸŒ', referral: 'ðŸ¤', cold_call: 'ðŸ“ž', event: 'ðŸŽª',
    social: 'ðŸ“±', partner: 'ðŸ¢', ad: 'ðŸ“¢', other: 'ðŸ“Œ',
};
...
const LeadList = () => {
    ...
    const handleConvert = async (e, lead) => {
        ...
        const confirmConvert = window.confirm(`Convert ${lead.first_name} ${lead.last_name} into a Client and Deal?`);
        if (!confirmConvert) return;
        
        setActionLoading(true);
        try {
            await crmService.convertLead(lead.id);
            import('react-hot-toast').then(m => m.default.success('Lead converted successfully!'));
            const data = await crmService.getLeads();
            setLeads(Array.isArray(data) ? data : data.results || []);
        } catch (error) {
            console.error(error);
                        ...
                        {filtered.length === 0 ? (
                            ...
                            return (
                                <tr key={lead.id} className="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors cursor-pointer">
                                    ...
                                    <td className="px-5 py-4 text-slate-400">{lead.company || 'â€”'}</td>
                                    <td className="px-5 py-4">
                                        <span className="flex items-center gap-1.5 text-slate-400">
                                            <span>{SOURCE_MAP[lead.source] || 'ðŸ“Œ'}</span>
