...
const ExportPage = () => {
    ...
    const handleExport = async () => {
        ...
        setExporting(true);
        try {
            const { analyticsService } = await import('../../../../core/api/analyticsService');
            for (const key of selected) {
                const res = await analyticsService.exportReport(key, 'csv');
                // The backend returns text/csv. We can create a blob.
                // Wait, exportReport returns response.data. If we need a blob, we must change how exportReport is called or implemented.
                // Since exportReport just does a normal get without responseType, it might return a string.
                const blob = new Blob([res], { type: 'text/csv' });
                const url = window.URL.createObjectURL(blob);
                const link = document.createElement('a');
                link.href = url;
                link.setAttribute('download', `bitguard-${key}-export.csv`);
