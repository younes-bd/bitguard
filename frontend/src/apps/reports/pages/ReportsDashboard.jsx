...
const ReportsDashboard = () => {
    ...
    const tiles = [
        { title: 'Revenue Analytics', desc: 'Financial health, MRR growth, and collection velocity', path: '/admin/reports/revenue', icon: TrendingUp, color: 'emerald', value: summary.monthlyRevenue != null ? `$${Number(summary.monthlyRevenue).toLocaleString()}` : undefined },
        { title: 'Finance & P&L', desc: 'Profit, loss, expenses, and A/R aging', path: '/admin/reports/finance', icon: DollarSign, color: 'emerald' },
        { title: 'Sales Performance', desc: 'Lead-to-deal funnel, win rate, and pipeline health', path: '/admin/reports/crm', icon: Users, color: 'blue', value: summary.winRate != null ? `${summary.winRate}% win rate` : undefined },
        { title: 'Service Desk Ops', desc: 'Ticket volume, SLA compliance, and support efficiency', path: '/admin/reports/support', icon: LifeBuoy, color: 'amber', value: summary.openTickets != null ? `${summary.openTickets} open` : undefined },
        { title: 'Security Posture', desc: 'Threat vectors, incident response, and risk mitigation', path: '/admin/reports/security', icon: ShieldAlert, color: 'rose', value: summary.openAlerts != null ? `${summary.openAlerts} active alerts` : undefined },
        { title: 'People & HR', desc: 'Headcount, certifications, and time utilization', path: '/admin/reports/hrm', icon: Users, color: 'blue' },
        { title: 'Project Management', desc: 'Active projects, budgets vs actuals', path: '/admin/reports/projects', icon: Activity, color: 'amber' },
    ];
