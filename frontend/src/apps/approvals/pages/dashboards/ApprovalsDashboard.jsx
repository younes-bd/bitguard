...
export default function ApprovalsDashboard() {
    ...
    const toggleHistory = async (id) => {
        ...
        if (!historyData[id]) {
            try {
                const res = await client.get(`approvals/requests/${id}/history/`);
                setHistoryData(prev => ({ ...prev, [id]: res.data?.results || res.data || [] }));
            } catch (err) {
                // If endpoint doesn't exist yet, simulate empty history
                        ...
                        {filteredRequests.map((r) => (
                            ...
                            >
                                            ...
                                            <div className="flex items-center gap-2">
                                                <button 
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        handleAction(r.id, 'reject', comment);
                                                    }}
                                ...
                                <div className="mt-4 border-t border-slate-800/50 pt-4">
                                    <button onClick={() => toggleHistory(r.id)} className="text-xs font-bold text-orange-400 hover:text-orange-300 transition-colors flex items-center gap-1">
                                        {expandedId === r.id ? 'Hide Approval Chain' : 'View Approval Chain'} <ArrowRight size={14} className={expandedId === r.id ? 'rotate-90' : ''} />
                                    </button>
                                    {expandedId === r.id && (
                                        <div className="mt-4 pl-4 border-l-2 border-slate-800 space-y-4">
                                            {(historyData[r.id] || []).length === 0 ? (
                                                <p className="text-xs text-slate-500">No multi-level chain history available.</p>
                                            ) : (
                                                historyData[r.id].map((step, i) => (
                                                    <div key={i} className="flex items-start gap-3 relative">
                                                        <div className={`w-2 h-2 rounded-full mt-1.5 ${step.status === 'approved' ? 'bg-emerald-500' : step.status === 'rejected' ? 'bg-rose-500' : 'bg-slate-500'}`} />
                                                        <div>
                                                            <p className="text-sm text-white font-medium">{step.approver_name || 'Unknown Approver'} <span className="text-xs text-slate-500 ml-2">{step.status}</span></p>
                                                            <p className="text-xs text-slate-500">{new Date(step.created_at || Date.now()).toLocaleString()}</p>
                                                            {step.comments && <p className="text-xs text-slate-400 italic mt-1">"{step.comments}"</p>}
                                                        </div>
                                                    </div>
                                                ))
                                            )}
