            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-white font-['Oswald'] tracking-wider uppercase flex items-center gap-3">
                        <DollarSign className="text-emerald-500" size={28} /> Customer Payments
                    </h1>
                    <p className="text-slate-400 text-sm mt-0.5">Track and reconcile incoming payments</p>
                </div>
                <div className="flex gap-2">
                    <button 
                        onClick={() => setShowForm(true)}
                        className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-sm font-bold transition-all flex items-center gap-2 shadow-lg shadow-emerald-600/20"
                    >
            ...
            <div className="flex flex-col sm:flex-row gap-4 bg-slate-900 border border-slate-800 p-4 rounded-xl">
                <div className="relative flex-1">
                    ...
                    <input
                        ...
                        className="w-full bg-slate-950 border border-slate-700 rounded-lg pl-10 pr-4 py-2 text-slate-200 focus:outline-none focus:border-emerald-500 transition-colors"
                        value={searchTerm}
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xl">
                {loading ? (
                            ...
                            <tbody className="divide-y divide-slate-800/50">
                                {filteredPayments.map((p) => (
                                    <tr key={p.id} className="group hover:bg-slate-800/30 transition-colors">
                                        ...
                                        <td className="p-4 font-semibold text-white">{p.partner_name || 'Walk-in Customer'}</td>
                                        ...
                                        <td className="p-4 text-right font-bold text-emerald-400">${parseFloat(p.amount).toFixed(2)}</td>
                    ...
                    <div className="text-center py-20 text-slate-500">
                        ...
                        <p className="font-bold text-slate-400">No inbound payments found.</p>
                    </div>
                )}
            </div>
