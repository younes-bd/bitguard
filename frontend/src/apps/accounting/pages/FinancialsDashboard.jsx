...
const FinancialsDashboard = () => {
    ...
    const totalExpenses = metrics?.expense_total || 0;
    const netProfit = totalRevenue - totalExpenses;
    const pendingInvoices = metrics?.overdue_count || 0;
    ...
    const stats = [
        ...
        { label: 'Overdue Invoices', value: pendingInvoices.toString(), change: 'Unpaid', icon: FileText, color: 'text-amber-400', bg: 'bg-amber-500/10' },
    ];
    ...
    const rawTransactions = [
        ...
        ...expenses.map(e => ({
            id: `e-${e.id}`, desc: e.title || e.category || 'Expense', date: e.incurred_date || e.created_at, amount: parseFloat(e.amount), type: 'expense'
        }))
    ];
    ...
    // If no data, provide an empty state
