...
const AgedReceivables = () => {
    ...
    const fetchReport = async () => {
        setLoading(true);
        try {
            const data = await erpService.getAgedReceivables();
            setReportData(data || []);
        } catch (error) {
        ...
        <div className="p-6 lg:p-8 space-y-6 animate-in fade-in duration-500">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1">Track outstanding customer invoices by age bucket.</p>
                </div>
                            ...
                            {loading ? (
                                ...
                                filteredData.map((row, idx) => (
                                    <tr key={idx} className="hover:bg-slate-800/30 transition-colors">
                                        <td className="px-6 py-4 font-medium text-slate-200">{row.client_name}</td>
                                        ...
                                        <td className="px-6 py-4 text-right text-emerald-400 font-bold">{formatCurrency(row.total)}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
                        {!loading && filteredData.length > 0 && (
                            <tfoot className="bg-slate-800/80 border-t-2 border-slate-700">
                                <tr>
                                    ...
                                    <td className="px-6 py-4 text-right font-bold text-emerald-500">{formatCurrency(totals.total)}</td>
                                </tr>
                            </tfoot>
                        )}
                    </table>
