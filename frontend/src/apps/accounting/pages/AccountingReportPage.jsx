...
export default function AccountingReportPage() {
    ...
    useEffect(() => {
        const fetchErp = async () => {
            try {
                const [invoicesData, journalsData, accountsData] = await Promise.all([
                    erpService.getInvoices(),
                    erpService.getJournalEntries(),
                    erpService.getAccounts()
                ]);
                setLedgers(Array.isArray(invoicesData) ? invoicesData : invoicesData?.results || []);
                setJournalEntries(Array.isArray(journalsData) ? journalsData : journalsData?.results || []);
    ...
    const cashFlow = journalEntries.reduce((sum, entry) => sum + (parseFloat(entry.amount) || 0), 0);
    const outstandingInvoices = ledgers.filter(l => l.status !== 'Paid' && l.status !== 'paid').length;
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                    ...
                    <div className="text-2xl font-bold text-white">${cashFlow.toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
