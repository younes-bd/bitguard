...
const InvoiceEdit = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (invoiceRes) {
                    setFormData({
                        client: invoiceRes.client || invoiceRes.client_id || '',
                        type: invoiceRes.type || 'standard',
                        project: invoiceRes.project || '',
                        contract: invoiceRes.contract || '',
                        issue_date: invoiceRes.issue_date || new Date().toISOString().split('T')[0],
                        payment_terms: 'custom',
                        due_date: invoiceRes.due_date || '',
                        status: invoiceRes.status || 'draft',
                        reference: invoiceRes.reference || '',
                        notes: invoiceRes.notes || '',
                        currency: invoiceRes.currency || 'USD',
                        discount_type: invoiceRes.discount_type || 'amount',
                        discount_value: invoiceRes.discount_value || 0,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            toast.error('Failed to update invoice. Please check all fields.');
            setLoading(false);
        }
    };
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8 pb-20 animate-in fade-in slide-in-from-top-4 duration-500">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-3xl font-bold text-white tracking-tight">Edit Document</h1>
                    <p className="text-slate-400">Update a standard invoice, proforma, or credit note.</p>
                </div>
            </div>
