...
import { Building2, Plus, Search, ChevronRight, Check } from 'lucide-react';
import client from '@/core/api/client';
import TenantManagerModal from '../details/TenantManagerModal';
...
const TenantList = () => {
    ...
    const fetchTenants = () => {
        ...
        client.get('tenants/')
            .then(r => { 
                const data = r.data?.data?.tenants ?? r.data?.results ?? r.data;
    ...
    return (
        <div className="p-6 lg:p-8 space-y-6 animate-in fade-in duration-400">
            ...
            <div className="relative">
                ...
                <input type="text" placeholder="Search tenants by name or domain..." value={search} onChange={e => setSearch(e.target.value)}
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="border-b border-slate-800 bg-slate-800/20">
                            {['Tenant Name', 'Domain', 'Plan', 'Modules', 'Users', 'Status', 'Joined', ''].map(h => (
                    ...
                    <tbody>
                        {loading ? (
                            <tr><td colSpan={8} className="py-16 text-center text-slate-500">Loading tenants...</td></tr>
                        ) : filtered.length === 0 ? (
                            ...
                            <tr key={t.id} className="border-b border-slate-800/60 hover:bg-slate-800/30 transition-colors group cursor-pointer" onClick={() => openEditModal(t)}>
                                ...
                                <td className="px-5 py-4 text-slate-400 font-mono text-xs">{t.domain ?? '—'}</td>
                                <td className="px-5 py-4">
                                    <span className="px-2 py-1 rounded bg-slate-800 text-xs font-medium text-slate-300 capitalize border border-slate-700">{t.subscription_plan ?? 'Free'}</span>
                                </td>
                                <td className="px-5 py-4 text-slate-400 font-mono text-xs">
                                    {t.allowed_modules?.length ? `${t.allowed_modules.length} Apps` : '—'}
                                </td>
                                <td className="px-5 py-4 text-slate-400">
                                    {t.user_count !== undefined ? t.user_count : '—'}
                                </td>
                                <td className="px-5 py-4">{statusBadge(t.status ?? (t.is_active ? 'active' : 'suspended'))}</td>
                                <td className="px-5 py-4 text-slate-400">{t.created_at?.split('T')[0] ?? '—'}</td>
                                <td className="px-5 py-4">
                                    <button className="text-slate-500 group-hover:text-violet-400 transition-colors">
