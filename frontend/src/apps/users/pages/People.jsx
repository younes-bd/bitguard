...
const People = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                if (activeTab === 'people') {
                    const data = await userService.getConnections();
                    setConnections(Array.isArray(data) ? data : []);
                } else {
                    const data = await userService.getSharedResources();
    ...
    const handleAccept = async (id) => {
        try {
            await userService.acceptConnection(id);
            // Refresh
            const data = await userService.getConnections();
            setConnections(Array.isArray(data) ? data : []);
        } catch (err) {
    ...
    const handleInvite = async (e) => {
        ...
        try {
            await userService.sendInvite(inviteEmail);
            alert("Invite sent!");
