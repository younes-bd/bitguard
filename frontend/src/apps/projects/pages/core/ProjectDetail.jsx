...
const ProjectDetail = () => {
    ...
    useEffect(() => {
        const fetchDetails = async () => {
            try {
                const [projData, tasksData, teamData] = await Promise.all([
                ...
                <div className="space-y-6">
                    <div className="glass-panel p-6 rounded-xl border border-slate-700/50">
                        ...
                        <div className="space-y-3">
                            {team.length === 0 ? (
                                <div className="text-slate-400 text-sm">No team members assigned.</div>
                            ) : (
                                team.map(member => (
                                    <div key={member.id} className="flex items-center space-x-3">
                                        <div className="w-8 h-8 rounded-full bg-slate-700 flex items-center justify-center text-xs text-white">
                                            {member.full_name ? member.full_name[0] : (member.username ? member.username[0] : 'U')}
                                        </div>
                                        <div>
