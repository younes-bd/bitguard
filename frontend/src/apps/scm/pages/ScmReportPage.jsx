        ...
        const fetchScmData = async () => {
            try {
                ...
                const [posData, vendorsData] = await Promise.all([
                    scmService.getPurchaseOrders(),
                    scmService.getVendors()
                ]);
                
                const posArr = Array.isArray(posData) ? posData : posData.results || [];
                const vendorsArr = Array.isArray(vendorsData) ? vendorsData : vendorsData.results || [];
                    ...
                    <tbody className="divide-y divide-slate-800">
                        ...
                        {orders.map((po, idx) => (
                            <tr key={idx} className="hover:bg-slate-800/30 transition-colors">
                                <td className="p-4 text-white font-medium">#{po.id}</td>
                                <td className="p-4">
