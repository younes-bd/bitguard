...
import { Files, Upload, Archive, Loader2, Download, Filter, Search, MoreVertical, FileText, User, Calendar, Trash2, Shield, History, AlertTriangle, Folder, FolderPlus, Tag, LayoutGrid, List } from 'lucide-react';
...
export default function EDMSDashboard() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            // We can also pass source_module and source_id directly to the API to reduce payload
            const params = {};
            if (sourceModule) params.source_module = sourceModule;
            if (sourceId) params.source_id = sourceId;
            
            const [docsRes, wsRes, tagsRes] = await Promise.all([
                edmsService.getDocuments(params),
                edmsService.getWorkspaces(),
                edmsService.getTags()
            ]);
    ...
    const filteredDocs = docs.filter(doc => {
        const matchesSearch = (doc.attachment?.name || '').toLowerCase().includes(searchTerm.toLowerCase());
        const matchesWorkspace = activeWorkspace === 'all' || doc.workspace_id === activeWorkspace;
        const matchesTag = activeTag === 'all' || doc.tags?.some(t => t.id === activeTag);
        
        let matchesUrlFilter = true;
        if (sourceModule && sourceId) {
            // If source and id are specified, only show matching docs regardless of other tabs
            matchesUrlFilter = doc.source_module === sourceModule && doc.source_id === sourceId;
        } else if (urlFilter === 'my') {
            if (user) matchesUrlFilter = doc.owner === user.id;
        } else if (urlFilter === 'archived') {
            matchesUrlFilter = doc.is_archived === true;
        } else if (urlFilter === 'recent') {
            // Very naive recent check
            const docDate = new Date(doc.created_at);
            const now = new Date();
            const diffTime = Math.abs(now - docDate);
            const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
            matchesUrlFilter = diffDays <= 7;
        } else {
            // if no filter, don't show archived by default
            matchesUrlFilter = doc.is_archived !== true;
        }

        return matchesSearch && matchesWorkspace && matchesTag && matchesUrlFilter;
    });
    ...
    const handleTogglePrivate = async (doc) => {
        try {
            await edmsService.updateDocument(doc.id, { is_private: !doc.is_private });
            fetchData();
        } catch (err) {
    ...
    const handleDelete = async () => {
        ...
        try {
            await edmsService.deleteDocument(docToDelete);
            setDocToDelete(null);
            fetchData();
        } catch (err) {
    ...
    const createWorkspace = async () => {
        if (newWorkspaceName.trim()) {
            await edmsService.createWorkspace({ name: newWorkspaceName.trim() });
            setNewWorkspaceName('');
                                    ...
                                    return (
                                        <div key={doc.id} className="bg-rose-500/5 hover:bg-rose-500/10 transition-colors border border-rose-500/20 rounded-xl p-4 flex items-start gap-4">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <Link to={`/admin/edms/documents/${doc.id}`} className="text-white font-semibold text-sm hover:text-rose-400 transition-colors truncate block">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 relative">
                        ...
                        {filteredDocs.map(d => (
                            <div key={d.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 hover:border-cyan-500/50 transition-all group flex flex-col shadow-xl">
                                ...
                                <Link to={`/admin/edms/documents/${d.id}`} className="text-lg font-bold text-white group-hover:text-cyan-400 transition-colors line-clamp-1 mb-1">
                                    ...
                                    <tr key={d.id} className="group hover:bg-cyan-500/5 transition-all">
                                        <td className="p-5">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <Link to={`/admin/edms/documents/${d.id}`} className="text-white font-bold group-hover:text-cyan-400 transition-colors text-base">{d.attachment?.name || 'Untitled'}</Link>
