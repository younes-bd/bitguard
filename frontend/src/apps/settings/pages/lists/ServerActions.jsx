...
export default function ServerActions() {
    ...
    const handleSave = async () => {
        ...
        try {
            if (editingAction?.id) {
                await client.patch(`base_setup/scheduled-actions/${editingAction.id}/`, form);
                toast.success('Server action updated');
            } else {
                await client.post('base_setup/scheduled-actions/', form);
    ...
    const handleDelete = async (id, name) => {
        ...
        try {
            await client.delete(`base_setup/scheduled-actions/${id}/`);
    ...
    const handleRun = async (id, name) => {
        setRunningId(id);
        try {
            const res = await client.post(`base_setup/scheduled-actions/${id}/run/`);
            ...
            <div className="mb-6 p-4 bg-blue-500/10 border border-blue-500/30 rounded-xl flex items-start gap-3">
                ...
                <div>
                    ...
                    <div className="text-sm text-slate-400">
                        ...
                        This module is being connected to the backend — available soon.
                    </div>
                </div>
            </div>
