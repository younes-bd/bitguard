...
export default function UserGroups() {
    ...
    const fetchRoleUsers = async (roleId) => {
        ...
        try {
            const res = await client.get(`iam/roles/${roleId}/users/`);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (editingGroup?.id) {
                await client.patch(`iam/roles/${editingGroup.id}/`, payload);
                toast.success('Group updated');
                if (selected?.id === editingGroup.id) {
                    setSelected(prev => ({ ...prev, ...payload }));
                }
            } else {
                await client.post('iam/roles/', payload);
    ...
    const handleDelete = async (group) => {
        ...
        try {
            await client.delete(`iam/roles/${group.id}/`);
    ...
    const openAssignUsers = async () => {
        ...
        try {
            const res = await client.get('iam/'); // Fetch all users
    ...
    const handleAssignUsers = async () => {
        ...
        setSaving(true);
        try {
            await client.post(`iam/roles/${selected.id}/assign_users/`, {
        ...
        <div className="p-6 lg:p-8 space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-400 text-sm mt-0.5">Manage roles and access groups — {safe.length} group{safe.length !== 1 ? 's' : ''}</p>
                </div>
                ...
                <div className="flex-1 bg-slate-900 border border-slate-800 rounded-xl overflow-hidden min-h-[500px]">
                    ...
                    {loading ? (
                        <div className="flex items-center justify-center py-20 gap-3 text-slate-500">
                            <Loader2 size={20} className="animate-spin" /> Loading groups…
                        </div>
                    ) : filtered.length === 0 ? (
