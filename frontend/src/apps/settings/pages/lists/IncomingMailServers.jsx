...
export default function IncomingMailServers() {
    ...
    const fetchServers = async () => {
        setLoading(true);
        try {
            const res = await client.get('base_setup/mail-servers-incoming/');
    ...
    const handleSave = async () => {
        ...
        try {
            if (editingServer) {
                await client.patch(`base_setup/mail-servers-incoming/${editingServer.id}/`, form);
                toast.success('Server updated');
            } else {
                await client.post('base_setup/mail-servers-incoming/', form);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await client.delete(`base_setup/mail-servers-incoming/${id}/`);
    ...
    const handleFetchNow = async (id, name) => {
        setFetchingId(id);
        try {
            await client.post(`base_setup/mail-servers-incoming/${id}/fetch_now/`);
