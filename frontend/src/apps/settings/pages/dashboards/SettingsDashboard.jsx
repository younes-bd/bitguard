...
import { Activity, Server, Users, AlertTriangle, Settings, Shield, RefreshCw, CheckCircle2, Globe } from 'lucide-react';
...
const SettingsDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      const metricsRes = await settingsService.getSystemMetrics();
      if (metricsRes.data) setMetrics(metricsRes.data);
      
      const auditRes = await settingsService.getAuditLogs({ limit: 5 });
  ...
  const handleDownloadReport = async () => {
    try {
      const response = await settingsService.generateReport();
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'settings_platform_report.txt');
    ...
    <div className="p-8 max-w-7xl mx-auto animate-fade-in-up relative">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex space-x-3">
          <Link to="/admin/settings/general" className="flex items-center px-4 py-2 bg-slate-900 border border-slate-800 text-slate-300 rounded-xl hover:bg-slate-850 hover:text-white transition-colors shadow-sm font-medium">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <MetricCard onClick={() => navigate('/admin/users')} title="Active Connections" value={metrics.active_users?.toLocaleString() || '0'} icon={Users} colorClass="bg-blue-500/10 text-blue-400 border-blue-500/20" />
        <MetricCard onClick={() => navigate('/admin/settings/companies')} title="Active Companies" value={metrics.active_companies?.toLocaleString() || '0'} icon={Users} colorClass="bg-purple-500/10 text-purple-400 border-purple-500/20" />
        <MetricCard onClick={() => navigate('/admin/apps')} title="Installed Apps" value={metrics.installed_apps?.toLocaleString() || '0'} icon={Settings} colorClass="bg-emerald-500/10 text-emerald-400 border-emerald-500/20" />
        <MetricCard onClick={() => setTelemetryModal('health')} title="System Health" value={metrics.system_health || '0%'} icon={Shield} colorClass="bg-emerald-500/10 text-emerald-400 border-emerald-500/20" />
        <MetricCard onClick={() => setTelemetryModal('health')} title="Global Uptime" value={metrics.server_uptime} icon={Server} colorClass="bg-amber-500/10 text-amber-400 border-amber-500/20" />
        <MetricCard onClick={() => navigate('/admin/settings/general?tab=integrations')} title="Active Webhooks" value={metrics.active_webhooks?.toLocaleString() || '0'} icon={Activity} colorClass="bg-blue-500/10 text-blue-400 border-blue-500/20" />
        ...
        <MetricCard onClick={() => navigate('/admin/settings/general?tab=retention')} title="Last Backup" value={metrics.last_backup ? new Date(metrics.last_backup).toLocaleDateString() : 'Never'} icon={Server} colorClass="bg-slate-500/10 text-slate-400 border-slate-500/20" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-sm p-6 h-fit">
          ...
          <div className="space-y-3">
             <button onClick={() => handleAction('cache', settingsService.clearCache)} disabled={loadingAction} className="w-full flex items-center justify-between p-4 rounded-xl border border-slate-800 bg-slate-950/20 hover:bg-slate-800/40 hover:border-slate-700 transition-colors group disabled:opacity-50 cursor-pointer">
             ...
             <button onClick={() => handleAction('index', settingsService.syncIndexes)} disabled={loadingAction} className="w-full flex items-center justify-between p-4 rounded-xl border border-slate-800 bg-slate-950/20 hover:bg-slate-800/40 hover:border-slate-700 transition-colors group disabled:opacity-50 cursor-pointer">
                <div className="flex items-center text-slate-300 font-medium text-sm">
             ...
             <button onClick={() => handleAction('maint', settingsService.toggleMaintenance)} disabled={loadingAction} className="w-full flex items-center justify-between p-4 rounded-xl border border-red-950/50 bg-slate-950/20 hover:bg-red-950/20 hover:border-red-900/40 transition-colors group disabled:opacity-50 cursor-pointer">
      ...
      {/* Odoo-style Configuration Menus */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-8">
        
        {/* Users & Companies */}
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-sm">
          <h2 className="text-lg font-bold text-white mb-6 flex items-center font-['Oswald'] tracking-wide">
             <Users className="w-5 h-5 mr-3 text-blue-400" /> USERS & COMPANIES
          </h2>
          <div className="space-y-1">
            <Link to="/admin/settings/users" className="block px-3 py-2 text-sm text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-lg transition-colors">Users</Link>
            <Link to="/admin/settings/companies" className="block px-3 py-2 text-sm text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-lg transition-colors">Companies</Link>
            <Link to="/admin/settings/groups" className="block px-3 py-2 text-sm text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-lg transition-colors">Groups</Link>
            <Link to="/admin/settings/access-rights" className="block px-3 py-2 text-sm text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-lg transition-colors">Access Rights</Link>
            <Link to="/admin/settings/record-rules" className="block px-3 py-2 text-sm text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-lg transition-colors">Record Rules</Link>
            <Link to="/admin/settings/security-policy" className="block px-3 py-2 text-sm text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-lg transition-colors">Security Policy</Link>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-sm">
          <h2 className="text-lg font-bold text-white mb-6 flex items-center font-['Oswald'] tracking-wide">
             <Settings className="w-5 h-5 mr-3 text-purple-400" /> TECHNICAL
          </h2>
          <div className="space-y-1">
            <Link to="/admin/settings/scheduled-actions" className="block px-3 py-2 text-sm text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-lg transition-colors">Scheduled Actions</Link>
            <Link to="/admin/settings/email-servers-outgoing" className="block px-3 py-2 text-sm text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-lg transition-colors">Outgoing Mail Servers</Link>
            <Link to="/admin/settings/email-servers-incoming" className="block px-3 py-2 text-sm text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-lg transition-colors">Incoming Mail Servers</Link>
            <Link to="/admin/settings/email-templates" className="block px-3 py-2 text-sm text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-lg transition-colors">Email Templates</Link>
            <Link to="/admin/settings/sequences" className="block px-3 py-2 text-sm text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-lg transition-colors">Sequences</Link>
            <Link to="/admin/settings/server-actions" className="block px-3 py-2 text-sm text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-lg transition-colors">Server Actions</Link>
            <Link to="/admin/settings/server-logs" className="block px-3 py-2 text-sm text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-lg transition-colors">System Logs</Link>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-sm">
          <h2 className="text-lg font-bold text-white mb-6 flex items-center font-['Oswald'] tracking-wide">
             <Globe className="w-5 h-5 mr-3 text-emerald-400" /> TRANSLATIONS
          </h2>
          <div className="space-y-1">
            <Link to="/admin/settings/languages" className="block px-3 py-2 text-sm text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-lg transition-colors">Languages</Link>
          </div>
        </div>
