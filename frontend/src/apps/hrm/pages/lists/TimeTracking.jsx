...
const TimeTracking = () => {
    ...
    const [form, setForm] = useState({ employee: '', project: '', description: '', hours: '', entry_date: new Date().toISOString().split('T')[0], is_billable: true });
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setEmployees(Array.isArray(empsData) ? empsData : empsData?.results ?? []);
            setProjects(Array.isArray(projsData) ? projsData : projsData?.results ?? []);
        } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setShowModal(false);
            setForm({ employee: '', project: '', description: '', hours: '', entry_date: new Date().toISOString().split('T')[0], is_billable: true });
            toast.success('Time entry logged successfully');
            fetchData();
        } catch (err) {
            console.error('Failed to log time:', err);
            toast.error(err.response?.data?.message || 'Failed to log time entry. Please try again.');
        } finally {
            setSubmitting(false);
        }
    };
                        ...
                        <tbody className="divide-y divide-slate-800/50">
                            {entries.map(entry => (
                                <tr key={entry.id} className="hover:bg-slate-800/30 transition-colors">
                                    ...
                                    <td className="p-4 text-white font-medium text-sm">{entry.project_name || 'â€”'}</td>
