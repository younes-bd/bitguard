            ...
            {expiringSoon.length > 0 && (
                <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 flex items-start gap-4 animate-in slide-in-from-top duration-500">
                    ...
                    <div className="flex-1">
                        <h4 className="text-amber-400 font-bold text-sm">Action Required: Certification Expiry</h4>
                        <p className="text-amber-200/60 text-xs mt-1">
                            {expiringSoon.length} {expiringSoon.length === 1 ? 'certification is' : 'certifications are'} expiring within the next 90 days. Please notify the relevant employees for renewal.
                        </p>
                    </div>
                    <button onClick={() => {}} className="text-amber-500 hover:text-amber-400 text-xs font-bold uppercase tracking-widest">Details</button>
                </div>
            )}

            {certs.some(c => c.expiry_date && new Date(c.expiry_date) < new Date()) && (
                <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 flex items-start gap-4 animate-in slide-in-from-top duration-500">
                    <div className="bg-red-500/20 p-2 rounded-lg">
                        <AlertTriangle className="text-red-500" size={20} />
                    </div>
                    <div className="flex-1">
                        <h4 className="text-red-400 font-bold text-sm">Critical: Expired Credentials</h4>
                        <p className="text-red-200/60 text-xs mt-1">
                            One or more employee certifications have expired. This may impact compliance status.
                        </p>
                    </div>
                </div>
            )}
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" required value={form.issuer} onChange={e => setForm({...form, issuer: e.target.value})}
                                    placeholder="e.g. ISCÂ², Amazon Web Services" className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-pink-500" />
                            </div>
