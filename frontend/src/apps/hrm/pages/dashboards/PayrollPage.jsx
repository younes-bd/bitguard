                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-slate-950/60 text-slate-500 text-xs font-bold uppercase tracking-wider border-b border-slate-800">
                                ...
                                <th className="p-4 text-right">Tax (15%)</th>
                        ...
                        <tbody className="divide-y divide-slate-800/50">
                            {employees.map(emp => (
                                <tr key={emp.id} className="hover:bg-slate-800/30 transition-colors">
                                    ...
                                    <td className="p-4 text-slate-400 text-sm">{emp.department?.name || 'â€”'}</td>
                                    <td className="p-4 text-right text-emerald-400 font-bold text-sm">${parseFloat(emp.salary || 0).toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                                    <td className="p-4 text-right text-rose-400 text-sm">-${(parseFloat(emp.salary || 0) * 0.15).toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                                    <td className="p-4 text-right text-amber-400 text-sm">-${(parseFloat(emp.salary || 0) * 0.05).toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                                    <td className="p-4 text-right text-white font-black text-sm">${(parseFloat(emp.salary || 0) * 0.80).toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                                </tr>
                            ))}
                        </tbody>
                        <tfoot>
                            <tr className="bg-slate-950/80 border-t-2 border-slate-700">
                                ...
                                <td className="p-4 text-right text-emerald-400 font-bold text-lg">${totalMonthly.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                                <td className="p-4 text-right text-rose-400 font-bold text-lg">-${(totalMonthly * 0.15).toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                                <td className="p-4 text-right text-amber-400 font-bold text-lg">-${(totalMonthly * 0.05).toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                                <td className="p-4 text-right text-white font-black text-lg">${(totalMonthly * 0.80).toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
