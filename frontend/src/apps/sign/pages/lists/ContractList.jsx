...
const ContractListPage = () => {
        ...
        const fetchAll = async () => {
            try {
                const [conData, cliData, slaData] = await Promise.all([
                    ...
                    signService.getSlaTiers ? signService.getSlaTiers().catch(() => []) : Promise.resolve([])
                ]);
    ...
    const handleSave = async (formData) => {
        ...
        try {
            if (selectedContract) {
                await signService.updateContract(selectedContract.id, formData);
            } else {
                await signService.createContract(formData);
            }
            setIsModalOpen(false);
            const data = await signService.getContracts();
    ...
    const handleDelete = async () => {
        ...
        setActionLoading(true);
        try {
            await signService.deleteContract(selectedContract.id);
