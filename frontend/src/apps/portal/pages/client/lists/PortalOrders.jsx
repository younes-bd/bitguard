                            ...
                            : orders.length === 0 ? <tr><td colSpan={6} className="py-16 text-center text-slate-500">No orders yet</td></tr>
                                : orders.map(order => (
                                    <tr key={order.id} className="border-b border-slate-800/50 hover:bg-slate-800/20">
                                        ...
                                        <td className="px-5 py-3 text-white font-semibold">${Number(order.amount ?? order.total_amount ?? order.total ?? 0).toFixed(2)}</td>
                                            ...
                                            <div className="flex justify-end gap-3 items-center">
                                                {(order.status === 'draft' || order.status === 'sent') && (
                                                    <button 
                                                        onClick={async () => {
                                                            try {
                                                                await portalService.acceptQuote(order.id);
                                                                const updated = await portalService.getOrders();
                                                                setOrders(Array.isArray(updated) ? updated : (updated?.results ?? []));
                                                            } catch (e) {
                                                                console.error("Failed to accept quote");
                                                            }
                                                        }}
                                                        className="bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30 px-3 py-1 rounded-md text-xs font-semibold flex items-center gap-1 transition-colors"
                                                    >
                                                        Accept
                                                    </button>
                                                )}
