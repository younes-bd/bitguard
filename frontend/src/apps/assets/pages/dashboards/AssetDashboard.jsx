...
const AssetDashboard = () => {
    ...
    useEffect(() => {
        Promise.all([
            assetService.getStats().catch(() => ({ total: 0, active: 0, maintenance: 0, retired: 0, by_type: {} })),
            assetService.getAssets({ page_size: 8 }).catch(() => ({ results: [] })),
        ]).then(([s, a]) => {
            setStats(s);
        ...
        <div className="p-6 lg:p-8 space-y-8 animate-in fade-in duration-400">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
                    ...
                    <Link to="/admin/assets/assets" className="text-teal-400 hover:text-teal-300 text-sm font-medium flex items-center gap-1">
