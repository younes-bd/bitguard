...
export default function AssetDetail() {
    ...
    useEffect(() => {
        Promise.all([
            client.get(`assets/assets/${id}/`),
            client.get(`assets/assets/${id}/history/`).catch(() => ({ data: [] })),
    ...
    const handleAction = async (action) => {
        ...
        try {
            ...
            await client.patch(`assets/assets/${id}/`, { status });
    ...
    if (!asset) return (
        <div className="text-center py-20 text-slate-500">
            ...
            <button onClick={() => navigate('/admin/assets/assets')} className="mt-4 text-teal-400 text-sm hover:underline">Back to Assets</button>
        </div>
    );
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6 pb-12 px-4 sm:px-6 animate-in fade-in duration-500">
            <div className="flex items-center gap-4">
                <button onClick={() => navigate('/admin/assets/assets')} className="p-2 bg-slate-800 text-slate-400 hover:text-white rounded-xl transition-colors">
                    <ArrowLeft size={20} />
                </button>
                    ...
                    <button 
                        onClick={async () => {
                            try {
                                ...
                                const res = await reportingService.generateReport(null, 'assets.Asset', asset.id);
