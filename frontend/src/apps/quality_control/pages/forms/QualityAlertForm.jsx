...
const QualityAlertForm = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const url = isEdit ? `/api/quality_control/qualityalerts/${id}/` : `/api/quality_control/qualityalerts/`;
    ...
    }).then(() => navigate(`/admin/quality_control/qualityalerts`));
  };
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <button 
            onClick={() => navigate(`/admin/quality_control/qualityalerts`)}
            className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors"
          >
            <ArrowLeft className="w-5 h-5" />
          </button>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-200">
            {isEdit ? 'Edit QualityAlert' : 'New QualityAlert'}
          </h1>
        </div>
        <button 
...
export default QualityAlertForm;
