...
const QualityDashboard = () => {
  ...
  useEffect(() => {
    qualityService.getAlerts().then(data => {
      setAlerts(Array.isArray(data) ? data : data.results || []);
      setLoading(false);
    }).catch(() => setLoading(false));
  }, []);

  const openAlerts = alerts.filter(a => a.state === 'draft').length;
  const criticalAlerts = alerts.filter(a => a.priority === '3' && a.state !== 'close').length;
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group overflow-hidden relative">
          ...
          <h3 className="text-slate-500 dark:text-slate-400 font-medium mb-1">Total Alerts</h3>
          <p className="text-3xl font-bold text-slate-800 dark:text-white">{loading ? '...' : alerts.length}</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group overflow-hidden relative">
          ...
          <h3 className="text-slate-500 dark:text-slate-400 font-medium mb-1">Open Alerts</h3>
          <p className="text-3xl font-bold text-slate-800 dark:text-white">{loading ? '...' : openAlerts}</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group overflow-hidden relative">
          ...
          <h3 className="text-slate-500 dark:text-slate-400 font-medium mb-1">Critical Alerts</h3>
          <p className="text-3xl font-bold text-slate-800 dark:text-white">{loading ? '...' : criticalAlerts}</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl h-96 flex items-center justify-center relative overflow-hidden group">
        ...
        <div className="relative text-center space-y-4 max-w-md px-6">
          <div className="w-20 h-20 bg-emerald-100 dark:bg-emerald-500/20 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform duration-500">
            <LayoutDashboard className="w-10 h-10 text-emerald-500 dark:text-emerald-400" />
          </div>
