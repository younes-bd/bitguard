...
const PublicLayoutInner = ({ children }) => {
    ...
    const orgJsonLd = {
        ...
        "logo": "https://www.bitguard.com/assets/images/logo.png",
        "contactPoint": {
            "@type": "ContactPoint",
                        ...
                        <div className="flex items-center gap-1.5">
                            ...
                            {isAuthenticated ? (
                                <div className="hidden xl:flex items-center gap-1.5">
                                    ...
                                    {/* Avatar Dropdown — click-toggled, not hover-based */}
                                    <div className="relative">
                                        <button
                                            onClick={() => setOpenMenu(openMenu === 'user' ? null : 'user')}
                                            className="flex items-center gap-1.5 py-1 px-1.5 rounded-lg dark:hover:bg-slate-800 hover:bg-slate-100 transition-all cursor-pointer bg-transparent border-0"
                                            aria-expanded={openMenu === 'user'}
                                            aria-haspopup="true"
                                        >
                                            <img
                                                src={user?.avatar || "/assets/images/user/avatar.png"}
                                                alt="User"
                                                className="w-7 h-7 rounded-full object-cover border-2 dark:border-slate-600 border-slate-200"
                                                onError={(e) => { e.target.onerror = null; e.target.src = 'https://ui-avatars.com/api/?name=' + encodeURIComponent((user?.first_name || user?.email?.split('@')[0] || 'U')) + '&background=2563EB&color=fff&size=64'; }}
                                            />
                                            <i className={`bi bi-chevron-down text-[10px] dark:text-slate-400 text-slate-500 transition-transform duration-200 ${openMenu === 'user' ? 'rotate-180' : ''}`}></i>
                                        </button>
                                        {openMenu === 'user' && (
                                            <div className="absolute right-0 top-full mt-2 w-[220px] dark:bg-slate-900 bg-white border dark:border-slate-800 border-slate-200 rounded-xl shadow-2xl overflow-hidden z-[1000]">
                                                <div className="px-4 py-3 border-b dark:border-slate-800 border-slate-100 dark:bg-slate-800/40 bg-slate-50">
                                                    <div className="font-semibold dark:text-white text-slate-900 text-sm">{user?.first_name ? `${user.first_name} ${user.last_name || ''}`.trim() : (user?.email?.split('@')[0] || 'User')}</div>
                                                    <div className="text-xs dark:text-slate-400 text-slate-500 truncate mt-0.5">{user?.email}</div>
                                                </div>
                                                <div className="p-2">
                                                    {[
                                                        { to: '/admin', label: 'Dashboard', icon: 'bi-speedometer2', color: 'text-blue-500' },
                                                        { to: '/admin/crm', label: 'Staff Portal', icon: 'bi-people-fill', color: 'text-indigo-500' },
                                                        { to: '/admin/accounting', label: 'ERP Console', icon: 'bi-grid-3x3-gap-fill', color: 'text-green-500' },
                                                    ].map(item => (
                                                        <Link key={item.to} to={item.to} className="flex items-center gap-3 px-3 py-2 rounded-lg dark:hover:bg-slate-800 hover:bg-slate-50 dark:text-slate-300 text-slate-600 dark:hover:text-white hover:text-slate-900 transition-colors no-underline text-sm">
                                                            <i className={`bi ${item.icon} ${item.color} w-4 text-center`}></i>
                                                            {item.label}
                                                        </Link>
                                                    ))}
                                                    <div className="h-px dark:bg-slate-800 bg-slate-100 my-1.5"></div>
                                                    {[
                                                        { to: '/account/personal-info', label: 'My Profile', icon: 'bi-person-circle', color: 'text-slate-400' },
                                                        { to: '/account/security', label: 'Settings', icon: 'bi-gear', color: 'text-slate-400' },
                                                    ].map(item => (
                                                        <Link key={item.to} to={item.to} className="flex items-center gap-3 px-3 py-2 rounded-lg dark:hover:bg-slate-800 hover:bg-slate-50 dark:text-slate-300 text-slate-600 dark:hover:text-white hover:text-slate-900 transition-colors no-underline text-sm">
                                                            <i className={`bi ${item.icon} ${item.color} w-4 text-center`}></i>
                                                            {item.label}
                                                    ...
                                                    <div className="h-px dark:bg-slate-800 bg-slate-100 my-1.5"></div>
                                                    <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg dark:hover:bg-red-500/10 hover:bg-red-50 text-red-500 dark:hover:text-red-400 hover:text-red-600 transition-colors bg-transparent border-0 cursor-pointer text-left text-sm">
                                                        <i className="bi bi-box-arrow-right w-4 text-center"></i>
                                                        Sign Out
                                                    </button>
                                                </div>
                                            </div>
                                ...
                                <div className="hidden xl:flex items-center gap-2">
                                    <Link to="/store" className="flex items-center gap-1.5 px-3 py-1.5 dark:text-slate-300 text-slate-600 dark:hover:text-white hover:text-slate-900 text-[13px] font-medium transition-colors no-underline rounded-lg dark:hover:bg-slate-800 hover:bg-slate-100">
            ...
            <div className={`fixed inset-0 ${isDark ? 'bg-slate-950/95' : 'bg-white/95'} backdrop-blur-2xl z-40 transition-transform duration-500 ease-in-out xl:hidden ${mobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <div className="flex flex-col h-full pt-20 px-6 pb-10 overflow-y-auto">
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        {/* IT Store */}
                        <div className="py-3 border-b dark:border-slate-800/50 border-slate-200">
                            <Link to="/store" onClick={() => setMobileMenuOpen(false)} className="text-lg font-bold dark:text-white text-slate-900 no-underline block">IT Store & Hardware</Link>
                        </div>
                        ...
                        {!isAuthenticated ? (
                            ...
                            <div className="flex items-center justify-between p-4 bg-slate-900 rounded-xl border border-slate-800">
                                <div className="flex items-center gap-3">
                                    <img src={user?.avatar || "/assets/images/user/avatar.png"} alt="User" className="w-10 h-10 rounded-full border border-slate-700" onError={(e) => { e.target.onerror = null; e.target.src = 'https://ui-avatars.com/api/?name=' + encodeURIComponent((user?.first_name || user?.email?.split('@')[0] || 'U')) + '&background=2563EB&color=fff&size=64'; }} />
                                    <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-8 lg:gap-12">
                        ...
                        <div className="col-span-2 md:col-span-4 lg:col-span-2 pr-0 lg:pr-12 flex flex-col gap-8">
                            <Link to="/" className="flex items-center gap-[12px] no-underline group shrink-0">
                                <div className="relative">
                                    <div className="absolute inset-0 bg-blue-500 blur-md opacity-20 group-hover:opacity-40 transition-opacity rounded-full"></div>
                                    <img src="/assets/logo/logo.png" alt="BitGuard" className={`relative h-[30px] w-auto transition-all duration-300 ${isDark ? 'brightness-0 invert' : ''}`} onError={(e) => e.target.style.display = 'none'} />
                                </div>
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            <div className="flex flex-col gap-3">
                                ...
                                <Link to="/store" className={`text-[13px] no-underline transition-colors flex items-center gap-1 ${isDark ? 'hover:text-white text-slate-400' : 'hover:text-blue-600 text-slate-500'}`}>IT Store <i className="bi bi-box-arrow-up-right text-[10px]"></i></Link>
                            </div>
                        </div>
