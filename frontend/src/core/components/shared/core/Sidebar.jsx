...
const Sidebar = ({ title = "BitGuard", items = [], sections = [], backLink = null, collapsed, setCollapsed }) => {
    const location = useLocation();
    const { user } = useAuth();

    // Check if user is an admin 
    const isAdmin = user?.is_staff || user?.is_superuser || (user?.role?.name?.toLowerCase() === 'admin');

    // Combined menu: if sections are provided, use them, otherwise use flat items
    const menuData = sections.length > 0 ? sections : (items.length > 0 ? [{ items }] : []);
                ...
                {menuData.map((section, sIndex) => (
                    <React.Fragment key={`sec-${sIndex}`}>
                        {section.title && !collapsed && (
                            <div className="px-4 py-2 mt-4 text-[10px] font-black text-slate-500 uppercase tracking-[2px] border-l-2 border-blue-500/30 ml-2">
                                {section.title}
                            </div>
                        )}
                        {section.items.map((item, index) => {
                            ...
                            // Permission Check: If item has permissions array, and user is not admin, hide it
                            if (item.permissions && item.permissions.length > 0 && !isAdmin) {
                                return null;
                            }
