...
│   ├── billing/     Subscription plans, Stripe webhooks, SaaS billing, Dunning Management
...
│   └── portal/      Client Portal Admin View (Preview and impersonation)
│
├── Security Platform Layer        ← /api/security/
│   └── soc/         INTERNAL SOC: Alerts, Incidents, ThreatIntelligence, LogAnalysis
│                    COMPLIANCE: ComplianceCenter (SOC2, ISO27001, GDPR, HIPAA), RiskRegister
│                    SAAS PRODUCT: Workspaces, ManagedEndpoints (ITAM/MDR), CloudApps, SystemMonitors
│
├── Operations Layer
│   ├── hrm/         Employees, Departments, LeaveRequests, Certifications, TimeEntries, Payroll Metrics
│   ├── scm/         Vendors, InventoryItems, PurchaseOrders, PurchaseOrderLines
│   ├── itam/        Assets, Asset Depreciation, License Management
│   ├── documents/   Document Vault with Versioning, Access Control, and Expiry Alerts
│   └── approvals/   Authorization requests with nested Multi-Level Chains
│
├── Intelligence Layer
│   ├── dashboard/   BFF Command Center — aggregates KPIs, owns no tables
│   ├── reports/     Cross-module executive reporting (Finance, HR, Projects, SOC, CRM)
...
| `/api/billing/` | Subscription plans, Stripe billing, Dunning | ✅ |
| `/api/erp/` | Invoices, payments, expenses, projects | ✅ |
| `/api/crm/` | Clients, deals, leads, contacts, activities | ✅ |
| `/api/contracts/` | Service contracts, SLA tiers, quotes | ✅ |
| `/api/support/` | Tickets, messages | ✅ |
| `/api/security/` | SOC, Compliance, Risk, Security Platform | ✅ |
| `/api/hrm/` | Employees, leave, certifications, time, payroll | ✅ |
| `/api/scm/` | Vendors, inventory, purchase orders | ✅ |
| `/api/itam/` | Assets, depreciation, licenses | ✅ NEW |
| `/api/documents/`| Vault, versions, expiry alerts | ✅ NEW |
| `/api/approvals/`| Multi-level approval chains | ✅ NEW |
| `/api/portal/` | Client portal settings and views | ✅ NEW |
| `/api/reports/` | Executive and cross-module aggregated reports | ✅ NEW |
...
│   ├── documents/      # Vault, expiry alerts
│   ├── erp/            # Invoices, expenses, projects
│   ├── hrm/            # HR dashboard, Payroll, Leave
│   ├── itam/           # Assets, Depreciation, Licenses
│   ├── marketing/      # Campaigns
│   ├── portal/         # Portal Dashboard, Client Portal View
│   ├── reports/        # Executive Summary, Finance, HRM, Projects, Security
│   ├── scm/            # Inventory, vendors
│   ├── soc/            # Security Operations, Compliance Register, Risks
